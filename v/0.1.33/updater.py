...
def check_and_download() -> dict:
    ...
    with _LOCK:
        # 확인 중 플래그가 오래 걸려 있으면(스레드가 죽었거나 네트워크가 물림) 풀어 준다.
        # 안 그러면 설정 화면이 '확인하는 중…'에서 영영 못 벗어난다.
        if STATE["checking"] and time.time() - STATE.get("started_at", 0) < 180:
            return dict(STATE)
        STATE["checking"] = True
...
def start_background() -> None:
    ...
    if not LOCAL_MODE:
        return
    def loop():
        time.sleep(3)                           # 첫 화면이 뜬 직후 — 늦게 확인하면 배너도 그만큼 늦게 뜬다
        while True:
            try:
                check_and_download()
            except Exception:
