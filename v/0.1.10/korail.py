...
from paths import data_path, LOCAL_MODE
...
# → 셋(UA·빌드ID·토큰)을 실제 존재하는 조합(Android 14)으로 맞춘다. (2026-09-08)
DEVICE_MODEL = os.environ.get("KORAIL_DEVICE_MODEL", "SM-S928N")
DEVICE_OS = os.environ.get("KORAIL_DEVICE_OS", "14")
DEVICE_BUILD = os.environ.get("KORAIL_DEVICE_BUILD", "UP1A.231005.007")
...
def _build_candidates(korail: Korail, p: Dict[str, Any]) -> List[Tuple]:
    ...
    for train in trains:
        ...
        if not has_seat and not is_waiting and not is_standing:
            ...
            continue
        seat = "1" if (is_standing or train.has_general_seat()) else "2"
...
SEAT_ATT_BLOCKED = {"020", "021", "028", "032", "033", "051", "052", "055"}
...
_LAYOUT_ROUTES = [("서울", "부산"), ("용산", "광주송정"), ("청량리", "부전"), ("서울", "강릉")]
...
def _sample_train(korail: Korail, train_type: str, date: str, updown: str = "down", skip_no: str = ""):
    """해당 종류·방향의 '자리가 있는' 열차 1편을 찾는다(배치 샘플용).
    방향은 열차번호 홀짝으로 가른다(홀수=하행). 매진 열차는 호차 목록이 ERI411321로 실패하므로
    잔여석이 있는 편을 고른다."""
    fallback = None
    routes = _LAYOUT_ROUTES if updown == "down" else [(b, a) for a, b in _LAYOUT_ROUTES]
    for dep, arr in routes:
        try:
            trains, _ = search_pages(korail, dep, arr, date, "060000", 1200, max_pages=2)
        except Exception:
            continue
        for t in trains:
            if str(getattr(t, "train_type_name", "")).strip() != train_type:
                continue
            if train_updown(getattr(t, "train_no", "")) != updown:
                continue
            if skip_no and str(getattr(t, "train_no", "")) == str(skip_no):
                continue   # 다른 편성을 보고 싶을 때(중련 ↔ 단독)
...
def layout_cars(korail_fn, train_type: str, date: str, updown: str = "down") -> List[dict]:
    """열차 종류의 호차 구성(캐시). [{car_no, cls, total}]
    korail_fn: 로그인된 Korail을 만드는 함수 — 캐시 적중 시에는 호출하지 않는다."""
    key = f"cars|{train_type}|{updown}"
    hit = _layout_get(key)
    if hit is not None:
        return hit
    korail = korail_fn() if callable(korail_fn) else korail_fn
    t = _sample_train(korail, train_type, date, updown)
    if t is None:
    ...
    prev = _layout_get(key) or []          # 편성이 편마다 다르므로(중련/단독) 본 것을 합친다
    ...
    _layout_put(f"sample|{train_type}|{updown}", {"train_no": getattr(t, "train_no", ""), "date": date})
    return cars
...
def layout_seats(korail_fn, train_type: str, car_no: str, seat_class: str, date: str, updown: str = "down") -> dict:
    """열차 종류·호차의 좌석 배치(캐시). 판매 여부는 빼고 자리 모양만 담는다.
    좌석 방향(순/역)은 상행·하행에서 정반대이므로 샘플이 어느 방향이었는지 함께 남긴다."""
    cars = _layout_get(f"cars|{train_type}|{updown}")
    ...
    key = f"seats|{train_type}|{updown}|{seat_class}|{car_no}"
    hit = _layout_get(key)
    if hit is not None:
        return hit
    korail = korail_fn() if callable(korail_fn) else korail_fn
    ...
    try:
        ...
        t2 = _sample_train(korail, train_type, date, updown, skip_no=getattr(t, "train_no", ""))
        if t2 is None:
            raise
...
def my_tickets(korail: Korail) -> dict:
    ...
    try:
        ...
        for row in (j.get("reservation_list") or []) if j.get("strResult") == "SUCC" else []:
            ...
            # 목록 요약의 h_seat_no('057')는 더미 — 실제 호차·좌석·좌석별 할인명·정상가는 SelTicketInfo.tk_seat_info에 있다(2026-09-06 실측 16호차 15A)
            seat_pick, disc_names, std_price = "", [], 0
            try:
                ...
                dd = {"Device": korail._device, "Version": korail._version, "Key": korail._key, "h_rsv_chg_no": "00000",
                      "h_orgtk_wct_no": ti.get("h_orgtk_wct_no", ""), "h_orgtk_ret_sale_dt": ti.get("h_orgtk_ret_sale_dt", ""),
                      "h_orgtk_sale_sqno": ti.get("h_orgtk_sale_sqno", ""), "h_orgtk_ret_pwd": ti.get("h_orgtk_ret_pwd", "")}
                ...
                std_price = _n(tinfo.get("h_std_seat_prc"))
                picks = []
                for si in tinfo.get("tk_seat_info") or []:
                    ...
                    if si.get("h_dcnt_knd_nm"):
                        disc_names.append(si["h_dcnt_knd_nm"])
                if picks:
                ...
                pass
            out["tickets"].append({
                "discount": ", ".join(dict.fromkeys(disc_names)), "std_price": std_price,
                ...
                "seat_pick": seat_pick, "seats": _n(ti.get("h_seat_cnt")) or 1,
...
def _pay_amount(reserve_raw: dict) -> str:
    return str(reserve_raw.get("h_tot_prc") or reserve_raw.get("h_rsv_amt") or "")
