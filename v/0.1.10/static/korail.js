...
function switchTab(name) {
  ...
  if (name === 'settings' && typeof _loadMyInfo === 'function') _loadMyInfo();
...
function installVersionWatch() {
  ...
  const check = () => {
    ...
    checking = true;
    api('GET', '/version')
      .then(r => { if (r && r.ok && r.v && String(r.v) !== String(window.__ASSET_V)) location.reload(); })
      .catch(() => {})
...
function makeStationField(value, placeholder, onChange, stations) {
  ...
  const isValid = () => stationList.includes(inp.value.trim());
  function refresh() {
    const v = inp.value.trim();
    errEl.style.display = (touched && v && !stationList.includes(v)) ? '' : 'none';   // 칸에서 벗어난 뒤에만 표시
    if (onChange) onChange();
  }
  function show(list, headItem) {
    dd.innerHTML = '';
    const favs = _favLoad().filter(x => stationList.includes(x));
  ...
  const _allSorted = () => stationList.slice().sort((a, b) => a.localeCompare(b, 'ko'));
  inp.addEventListener('focus', () => {
    prev = inp.value.trim(); picked = false;
    inp.value = '';
    // '최근'은 실제 역일 때만(오타·초성 입력은 제외)
    show(_allSorted(), stationList.includes(prev) ? prev : null);
  });
  inp.addEventListener('input', () => {
    const v = inp.value.trim();
    show(v ? _filterStations(v, stationList) : _allSorted(),
         v ? null : (stationList.includes(prev) ? prev : null));
    if (v) refresh();
  });
...
function _tcFresh(key) {
  ...
  const ttl = e.preview ? 3600000 : _TC_TTL;
...
function makeCarChip(c, on, onClick) {
  const spec = c.cls === '2';
  const rest = (c.rest != null) ? c.rest : c.total;
  const ch = el('button', 'car-chip' + (on ? ' on' : '') + ((c.rest != null && c.rest <= 0) ? ' full' : '') + (spec ? ' spec' : ''));
  ch.type = 'button';
  ch.append(txt('span', 'car-chip-no', `${parseInt(c.car_no, 10)}호차`),
            txt('span', 'car-chip-sub', (c.rest != null ? `잔여 ${rest}` : `${rest || 0}석`)));
  ch.title = `${spec ? '특실' : '일반실'} ${parseInt(c.car_no, 10)}호차` + (c.total ? ` · ${rest}/${c.total}석` : '');
  if (onClick) ch.addEventListener('click', onClick);
  return ch;
}
...
const SEAT_ATT_BLOCKED = new Set(['020', '021', '028', '032', '033', '051', '052', '055']);
...
function _attLegend(seats) {
  ...
  return ' ' + kinds.map(a => (a === '019'
    ...
    : '<span class="seat-cell lg att-blocked"></span>') + SEAT_ATT_NAMES[a]).join(' ');
}

function renderSeatTable(host, seats, opts) {
  const o = opts || {};
  // 코레일 응답 순서를 뒤집어 그린다 — 응답이 14→1이면 화면은 1→14 (2026-09-08 확인)
  const ordered = (seats || []).slice().sort((a, b) => (b.seq ?? 0) - (a.seq ?? 0));
  const cols = [...new Set(ordered.map(x => x.col))];
  ...
  cols.forEach((c, ci) => {
    ...
    rows.forEach(rn => {
      ...
      cell.textContent = badge || String(rn);
      ...
      if (att === '019') {           // 유아동반석 — 선택 가능, 아이콘만
        cell.classList.add('att-baby');
        const ic = document.createElement('span'); ic.className = 'seat-ic'; ic.innerHTML = _ICON_BABY;
        cell.appendChild(ic);
      }
...
function toast(msg) {
  ...
  el0.textContent = msg; el0.classList.add('on');
...
function _presetTitle(p) { return `${p.dep}→${p.arr} ${_WD[p.weekday] || ''}요일 ${_presetHour(p)}시 (${p.adults}명)`; }
...
function _presetLines(p) {
  return [`${p.dep}→${p.arr}`, `${_WD[p.weekday] || ''}요일 ${_presetHour(p)}시`];
}
...
function _trainLabel(t) { const ty = String(t.type || '').trim(); if (!t.no) return ty; return ty ? `${ty} (${t.no})` : String(t.no); }
...
function _buildModePanel(panel, cfg) {
  ...
  const isOpenRun = () => !!(typeof dateF !== 'undefined' && dateF && _dayInfo(dateF.getValue()) ) || !!(typeof dateF !== 'undefined' && dateF && !_isOpenNow(dateF.getValue()));
  ...
  // 역
  let _ensT = null;
  function _invalidateTrains() { _flashList(); _syncNcardRoute(); _clearFixed(); selTrains = []; carList = null; _trainFetchKey = null; _trainErr = '';
    if ((dateF.getValue() || '') === _todayStr() && startHour < new Date().getHours()) { startHour = _nearestHour(); hourSpan = 1; }
    renderHourChips(); renderTrainSel(); clearTimeout(_ensT); _ensT = setTimeout(() => _ensureTrains(_trainKey()).then(() => { timeRange.refreshLabel(); renderTrainSel(); }), 700); if (isOpenRun()) autoSchedule(); }
  const depF = makeStationField(saved.dep || cfg.defaultDep, '초성 입력', () => { _invalidateTrains(); save(); }, STATIONS);
  const arrF = makeStationField(saved.arr || cfg.defaultArr, '초성 입력', () => { _invalidateTrains(); save(); }, STATIONS);
  ...
  seatModeF = makeSegmentedField([['auto','긴급'],['cand','후보 등록'],['manual','직접 선택']],
  ...
  const hourF = makeSheetPicker(Array.from({ length: 24 }, (_, h) => [h, String(h).padStart(2, '0') + ':00']), startHour,
    v => { startHour = +v; hourSpan = 1; selTrains = []; _flashList(); renderTrainSel(); save(); });
  ...
  const seatF = makeSheetPicker(_SEAT_OPTS, saved.seatClass || 'general_first', () => save());
  ...
  const waitCb = makeCheckbox(!!saved.allowWaiting); waitCb.addEventListener('change', () => { save(); _syncEtcNotes(); });
  const _standMsg = '좌석이 매진이고 입석만 남은 열차도 자동예매합니다.\n⚠️ 실험적 기능: 좌석이 보장되지 않고 자동결제도 미적용입니다.';
  const standingCb = makeCheckbox(!!saved.allowStanding); standingCb.addEventListener('change', () => { save(); _syncEtcNotes(); });
  ...
  function renderTrainSel() {
    try { _renderTrainSel(); }
      ...
      again.addEventListener('click', () => { try { localStorage.removeItem('kor_ktx_traincache'); } catch {} _trainFetchKey = null; _trainErr = ''; renderTrainSel(); });
  ...
  function _renderTrainSel() {
    ...
    let rows = _tcList(key, a, b);
    // 최소 5편은 보이게 다음 시간대를 이어 붙인다(코레일은 자정~새벽 출발 열차가 없어 그날 23:59에서 멈춘다)
    while (rows && rows.length < 5 && startHour + hourSpan < 24) {
      hourSpan += 1;
      rows = _tcList(key, timeRange.getStart(), timeRange.getEnd());
    }
    ...
    if (!rows) {
      if (_trainFetchKey !== key) {
        _trainFetchKey = key; _trainErr = ''; _trainWaitFrom = Date.now();
        clearTimeout(_trainWaitT);
        // 응답이 오지 않아도 로딩만 계속 도는 일이 없게 한계 시간을 둔다
        _trainWaitT = setTimeout(() => {
          if (!_tcList(key, timeRange.getStart(), timeRange.getEnd()) && !_trainErr) {
            _trainErr = '열차 목록을 불러오지 못했습니다 (응답 지연). 다시 시도해 주세요.';
            renderTrainSel();
          }
        }, 15000);
        _ensureTrains(key)
          .then(r => { if (r && !r.ok) _trainErr = r.error || '조회 실패'; })
          .catch(() => { _trainErr = '조회 실패'; })
          .finally(() => { clearTimeout(_trainWaitT); renderTrainSel(); });
      }
      if (_trainErr) {
        body.appendChild(txt('div', 'train-list-msg', _trainErr));
        const again = el('button', 'train-more'); again.type = 'button'; again.textContent = '다시 시도';
        again.addEventListener('click', () => { _trainFetchKey = null; _trainErr = ''; renderTrainSel(); });
    ...
    if (startHour + hourSpan < 24) {
      const more = el('button', 'train-more'); more.type = 'button';
      const nh = String(startHour + hourSpan).padStart(2, '0');
      more.textContent = `${nh}시 이후 열차 더보기`;
      more.addEventListener('click', () => { hourSpan += 1; renderTrainSel(); });
      body.appendChild(more);   // 목록 안쪽 맨 아래(스크롤해야 보임)
    }
  }

  // ── 좌석 옵션: 보통=좌석맵(정확히 인원수) / 특수-감시=좌석맵(인원수 이상) / 특수-선호=선호 조건 ──
  const seatCard = makeGridCard();
  const sideF = makeSheetPicker(_SIDE_OPTS, _SIDE_OPTS.some(o => o[0] === saved.seatSide) ? saved.seatSide : 'window', () => { renderEtc(); save(); });
  const dirF = makeSheetPicker(_DIR_OPTS, _DIR_OPTS.some(o => o[0] === saved.seatDir) ? saved.seatDir : 'fwd', () => { renderEtc(); save(); });
  ...
  // 후보 등록: 어떤 템플릿을 쓸지(열차번호 자동 매칭 + 수동 변경)
  let candOverride = saved.candTemplate || '';
  const candPv = _candPicker();
  addRow(seatCard, 'cand', '후보 좌석', candPv.el, '설정 › 후보 좌석에서 등록한 우선순위대로 시도합니다. 열차번호가 같으면 자동으로 골라집니다.');
  // 좌석 배치는 열차 종류·진행 방향으로 정해진다(열차번호와 무관) — 그 둘로 템플릿을 고른다
  function _candTemplateFor(trainNo) {
    const list = _candLoad();
    if (candOverride) { const m = list.find(x => x.id === candOverride); if (m) return m; }
    const tr = (_tcList(_trainKey(), timeRange.getStart(), timeRange.getEnd()) || []).find(x => x.no === trainNo);
    const ty = tr ? String(tr.type || '').trim() : '';
    const ud = trainNo ? (parseInt(trainNo, 10) % 2 ? 'down' : 'up') : '';
    if (ty) { const m = list.find(x => x.trainType === ty && (!x.updown || x.updown === ud)); if (m) return m; }
    return null;
  }
  function _candPicker() {
    const w = div('picker-value'); const lab = span(''); lab.classList.add('u-picker-lab');
    const c = span(''); c.textContent = '▾'; c.classList.add('u-picker-sub');
    w.append(lab, c);
    const refresh = () => {
      const t = _candTemplateFor(fixed && fixed.train ? fixed.train.no : (selTrains[0] || null));
      lab.textContent = t ? `${t.trainType || 'KTX'} ${t.updown === 'up' ? '상행' : '하행'} (${(t.seats || []).length}순위)` : '등록 안 함';
      lab.style.color = t ? 'var(--blue)' : '#B0B8C1';
    };
    w.addEventListener('click', () => {
      const list = _candLoad();
      if (!list.length) { notify('설정 › 후보 좌석에서 먼저 등록하세요'); return; }
      const ov = div('picker-overlay'); const sheet = div('picker-sheet');
      sheet.appendChild(txt('div', 'wheel-title', '후보 좌석 선택'));
      const auto = div('picker-sheet-item train-pick' + (candOverride ? '' : ' selected'));
      auto.append(txt('span', 'train-name', '자동 (열차번호로 매칭)'));
      auto.addEventListener('click', () => { candOverride = ''; refresh(); save(); document.body.removeChild(ov); });
      sheet.appendChild(auto);
      list.forEach(t => {
        const it = div('picker-sheet-item train-pick' + (candOverride === t.id ? ' selected' : ''));
        it.append(txt('span', 'train-name', `${t.trainType || 'KTX'} ${t.updown === 'up' ? '상행' : '하행'}`),
                  txt('span', 'train-seat ok', `${(t.seats || []).length}순위`));
        it.addEventListener('click', () => { candOverride = t.id; refresh(); save(); document.body.removeChild(ov); });
        sheet.appendChild(it);
      });
      const bar = div('picker-sheet-cancel'); const cb = div('picker-sheet-cancel-btn'); cb.textContent = '취소';
      cb.addEventListener('click', () => document.body.removeChild(ov)); bar.appendChild(cb);
      ov.append(sheet, bar); ov.addEventListener('click', e => { if (e.target === ov) document.body.removeChild(ov); });
      document.body.appendChild(ov);
    });
    return { el: w, refresh };
  }
  ...
  function openEtcSheet() {
    ...
    row('좌석등급', seatF.el);
    if (seatMode() === 'auto') { row('창측 / 내측', sideF.el); row('진행 방향', dirF.el); }
  ...
  function renderFixed() {
    ...
    const { hw } = renderSeatTable(seatGrid, mapData.seats, {
      pending,
      dirNote: `열차 진행 방향 →  ${fixed.train.no}편 ${ud}`,
    ...
    if (_seatBoxH) hw.style.minHeight = _seatBoxH + 'px';
    requestAnimationFrame(() => { const h = hw.offsetHeight; if (h > 40) { _seatBoxH = h; hw.style.minHeight = ''; } });
  ...
  function _loadSkeleton(sync) {
    ...
    const hit = _layoutCache[ty + '|' + ud + '|' + car];
    if (hit && hit.ok) { f.skeleton = { seats: hit.seats || [] }; return true; }   // 같은 종류·방향의 표본(편성이 다를 수 있음)
    if (sync) return false;
    _layoutFetch(ty, car, ud).then(r => {
  ...
  function _prefetchLayouts(cars) {
    ...
    (async () => {
      for (const c of cars) {
        ...
        if (_layoutCache[ty + '|' + ud + '|' + no]) continue;
        try { await _layoutFetch(ty, no, ud); } catch {}
  ...
  { const cell = div('grid-cell-stack'); cell.appendChild(payWarn); runCard.appendChild(cell); }
  function _syncPayWarn() {
    const need = runF.getValue() === 'pay' && !payReady();
    payWarn.style.display = need ? '' : 'none';
  ...
  const btnStop = makePrimaryButton('', 'red', onStop); btnStop.classList.add('btn-run'); btnStop.innerHTML = _ICON_STOP + '<span>중단</span>'; btnStop.disabled = true;
  ...
  btnStop.style.display = 'none';   // 중단은 '결과' 탭에서
  ...
  function _startBlockReason() {
    ...
    if (seatMode() === 'cand') {
      const tpl = _candTemplateFor(fixed && fixed.train ? fixed.train.no : (selTrains[0] || null));
      if (!tpl || !(tpl.seats || []).length) return '후보 좌석을 먼저 등록하세요';
    }
  ...
  renderPresetBar = function () {
    ...
    if (!list.length) {
      ...
      box.appendChild(b);
      box.appendChild(txt('div', 'hint', '최근 3개월 이용 내역에서 자주 탄 구간과 시간대를 찾아 등록합니다'));
  ...
  function collect() { return { sub: sub(), dep: depF.getValue(), arr: arrF.getValue(), date: dateF.getValue(), startTime: timeRange.getStart(), endTime: timeRange.getEnd(), adults: adultF.getValue(), interval: ivlF.getValue(), seatClass: seatF.getValue(), seatMode: seatMode(), candTemplate: candOverride, allowWaiting: waitCb.checked, allowStanding: standingCb.checked, runMode: runF.getValue(), startMode, startAtDate, startAtTime, schedManual, trainNos: selTrains, seatSide: sideF.getValue(), seatDir: dirF.getValue(), startTime: String(startHour).padStart(2,'0') + ':00', fixed: fixed ? { train: fixed.train, car_no: fixed.car_no, seats: fixed.seats } : null, useNcard: !!_ncardActive() }; }
  ...
  if (fixed && fixed.train && !carList) loadCars();
  if (fixed && fixed.car_no && !fixed.map) loadSeatMap();
  ...
  function getStartBody() {
    ...
    if (isManual()) {
      ...
      // 후보 등록: 우선순위대로 좌석을 시도하고, 전부 없으면 설정한 폴백을 따른다
      const tpl = _candTemplateFor(fixed && fixed.train ? fixed.train.no : null);
      if (!tpl || !(tpl.seats || []).length) { notify('후보 좌석이 등록되지 않았습니다. 설정 › 후보 좌석에서 먼저 등록하세요'); return null; }
      base.seat_mode = 'cand';
      base.cand_seats = tpl.seats.slice(0, 10);
      base.cand_fallback = tpl.fallback === 'auto' ? 'auto' : 'watch';
      base.cand_label = tpl.name || '';
    } else {
    ...
    if (autoPay && method === 'cash' && !/^\d{6}$/.test(pin)) { notify('간편현금결제는 결제 정보에서 간편결제 PIN(6자리)을 입력해야 합니다'); _ktxPayModal(syncSetup); return null; }
    ...
    if (autoPay && method === 'cash' && pay.cashReceipt && !numVal.trim()) { notify('현금영수증 번호를 결제 정보에서 입력하세요'); _ktxPayModal(syncSetup); return null; }
    ...
    return Object.assign(base, {
      ...
      use_mileage: method === 'cash' && pay.useMileage === true,
  ...
  async function onStart() {
    ...
    try {
      ...
      _lastJob = job; _jobBodies[job] = body;
  ...
  async function onStop() { try { await api('POST', '/stop?mode=' + (_lastJob || 'urgent')); } catch (e) { notify('중단 실패: ' + e.message); } }
  _refreshStart();
}
...
function buildUsagePage() {
  ...
  _usageRow(c1, '회원번호', '코레일 회원번호를 입력하세요.');
...
function payReady() {
  const p = lsGetPay() || {};
  if (p.method === 'toss') return true;                      // 토스는 코레일에 등록된 키를 그대로 쓴다
  if (p.method === 'card') return _cardReady(p.card);        // 신용카드는 카드 정보가 다 있어야 한다
  return !!(p.payPin && String(p.payPin).length === 6);       // 간편현금결제는 PIN 6자리
}
...
const _PAY_METHODS = [['toss','토스페이'],['card','신용카드'],['cash','간편현금']];
function _payMethodLabel() { const m = lsGetPay().method || 'cash'; const hit = _PAY_METHODS.find(x => x[0] === m); return m === 'cash' ? '간편현금결제' : (hit ? hit[1] : '간편현금결제'); }

// 입력 모달 셸 (하단 취소/저장 — 저장 시 즉시 DB 반영)
function _editModal(title, buildBody, onSave) {
  const ov = div('picker-overlay'); const sheet = div('picker-sheet'); sheet.classList.add('u-sheet');
  sheet.appendChild(txt('div', 'wheel-title', title));
...
function _payFields(p) {
  const wrap = div('');
  const card = makeGridCard();
  const methodF = makeSegmentedField(_PAY_METHODS, p.method || 'cash', () => syncMethod());
  methodF.el.classList.add('seg-tight'); methodF.el.style.width = '240px';
  ...
  const dvPin = div('grid-divider-h'); const pinRow = div('grid-cell-full'); const pinAct = div('grid-cell-action'); pinAct.appendChild(pinInp); pinRow.append(txt('div','grid-cell-label','간편결제 PIN'), pinAct);
  card.append(dvPin, pinRow); wrap.appendChild(card);
  const mNote = _payMethodNote(() => methodF.getValue()); wrap.appendChild(mNote.el);
  wrap.appendChild(div('sp10'));
  const cardLabel = txt('div', 'sub-label', '카드 정보'); cardLabel.classList.add('u-pb4'); wrap.appendChild(cardLabel);
  const cardBlk = _cardSubcard(p); wrap.appendChild(cardBlk.el); wrap.appendChild(div('sp10'));
  const optLabel = txt('div', 'sub-label', '발권 · 할인'); optLabel.classList.add('u-pb4'); wrap.appendChild(optLabel);
  ...
  wrap.appendChild(optCard); wrap.appendChild(div('sp10'));
  const rcptLabel = txt('div', 'sub-label', '현금영수증'); rcptLabel.classList.add('u-pb4'); wrap.appendChild(rcptLabel);
  const receipt = _receiptSubcard(p); wrap.appendChild(receipt.el);
  ...
  function syncMethod() {
    const m = methodF.getValue(), cash = m === 'cash';
    [dvPin, pinRow, optLabel, optCard, rcptLabel, receipt.el].forEach(e => e.style.display = cash ? '' : 'none');
    [cardLabel, cardBlk.el].forEach(e => e.style.display = m === 'card' ? '' : 'none');
    mNote.paint();
  }
  syncMethod();
...
function _ktxAccountModal(onSaved) {
  ...
  _editModal('KTX 계정', (body) => {
    const card = makeGridCard();
    memInp = makeTextInput(a.memberNo || '', 150, '회원번호'); memInp.inputMode = 'numeric';
    memInp.addEventListener('input', () => { memInp.value = memInp.value.replace(/\D/g, ''); });
    pwInp = makePwInput(a.password || '', '비밀번호');
    appendGridFull(card, '회원번호', memInp); appendGridFull(card, '비밀번호', pwInp);
    body.appendChild(card);
  }, () => {
    const memberNo = memInp.value.trim(); if (!memberNo) { notify('회원번호를 입력하세요'); return false; }
...
const _PAY_NOTE_CASH = [
  '계좌 1회 등록으로 현금 결제를 할 수 있는 서비스입니다',
  '코레일 사이트(www.korail.com) - 마이페이지 - 결제관리 - 간편현금결제 설정에서 등록하세요',
].join('\n');
const _PAY_NOTE_TOSS_OK = '토스페이가 등록되어있습니다';
const _PAY_NOTE_TOSS_NONE = [
  '최초 1회 등록이 필요합니다.',
  '코레일 어플에서 승차권 결제 - 간편결제 - 토스페이로 1회 결제 해주세요',
].join('\n');
...
function _payMethodNote(getMethod) {
  const el0 = txt('div', 'note-line', '');
  el0.style.cssText = 'margin-top:9px;padding:0 4px;text-align:left';
  function paint() {
    const m = getMethod();
    if (m === 'cash') {
      el0.className = 'note-line'; el0.style.color = 'var(--text-muted)';
      el0.style.fontWeight = '500'; el0.textContent = _PAY_NOTE_CASH;
      el0.style.display = ''; return;
    }
    if (m !== 'toss') { el0.style.display = 'none'; return; }
    el0.style.display = ''; el0.style.fontWeight = '600'; el0.style.color = '';
    el0.className = 'note-line'; el0.textContent = '토스페이 등록 여부 확인 중…';
    el0.style.color = 'var(--text-muted)';
    _tossKeyState().then(ok => {
      if (getMethod() !== 'toss') return;          // 그 사이 수단을 바꿨으면 버린다
      if (ok === null) { el0.className = 'note-line'; el0.style.color = 'var(--text-muted)'; el0.textContent = '토스페이 등록 여부를 확인하지 못했습니다'; return; }
      el0.style.color = '';
      el0.className = ok ? 'note-line' : 'warn-line';
      el0.textContent = ok ? _PAY_NOTE_TOSS_OK : _PAY_NOTE_TOSS_NONE;
    });
  }
  return { el: el0, paint };
}
...
function _cardSubcard(p) {
  ...
  const noInp = makeTextInput(c.cardNo || '', 180, '숫자만 입력'); noInp.inputMode = 'numeric';
  noInp.addEventListener('input', () => {
    const d = noInp.value.replace(/\D/g, '').slice(0, 16);
    noInp.value = d.replace(/(.{4})/g, '$1 ').trim();
  });
  ...
  return { el: wrap, collect: () => ({
    cardNo: noInp.value.replace(/\D/g, ''),
...
function _ktxPayModal(onSaved) {   // 결제 정보 모달 — 저장은 계정 레코드의 pay로(lsSetPay)
  ...
  let methodF, pinInp, smartCb, milCb, receipt, dvPin, pinRow, optLabel, optCard, rcptLabel, cardLabel, cardBlk, mNote;
  _editModal('결제 정보', (body) => {
    // ① 결제 — 결제 수단 + (현금결제 시) PIN
    const card = makeGridCard();
    methodF = makeSegmentedField(_PAY_METHODS, p.method || 'cash', () => syncMethod());
    methodF.el.classList.add('seg-tight'); methodF.el.style.width = '240px';
    appendGridFull(card, '결제 수단', methodF.el);
    pinInp = makePwInput(p.payPin || '', '숫자 6자리', true, 6);
    pinInp.addEventListener('input', () => { pinInp.value = pinInp.value.replace(/\D/g, '').slice(0, 6); });
    dvPin = div('grid-divider-h'); pinRow = div('grid-cell-full'); const pinAct = div('grid-cell-action'); pinAct.appendChild(pinInp); pinRow.append(txt('div','grid-cell-label','간편결제 PIN'), pinAct);
    card.append(dvPin, pinRow);
    body.appendChild(card);
    mNote = _payMethodNote(() => methodF.getValue()); body.appendChild(mNote.el);
    body.appendChild(div('sp10'));
    // ①-b 카드 정보 (신용카드 전용)
    cardLabel = txt('div', 'sub-label', '카드 정보'); cardLabel.classList.add('u-pb4'); body.appendChild(cardLabel);
    cardBlk = _cardSubcard(p); body.appendChild(cardBlk.el); body.appendChild(div('sp10'));
    // ② 발권 · 할인 (간편현금결제 전용)
    optLabel = txt('div', 'sub-label', '발권 · 할인'); optLabel.classList.add('u-pb4'); body.appendChild(optLabel);
    ...
    optCard.appendChild(div('grid-divider-h'));
    const milRowEl = div('grid-cell-full'); const milAct = div('grid-cell-action'); milAct.appendChild(milCb); milRowEl.append(txt('div','grid-cell-label','마일리지 전액 사용'), milAct); optCard.appendChild(milRowEl);
    body.appendChild(optCard); body.appendChild(div('sp10'));
    // ③ 현금영수증 (간편현금결제 전용)
    rcptLabel = txt('div', 'sub-label', '현금영수증'); rcptLabel.classList.add('u-pb4'); body.appendChild(rcptLabel);
    receipt = _receiptSubcard(p); body.appendChild(receipt.el);
    function syncMethod() {
      const m = methodF.getValue(), cash = m === 'cash';
      [dvPin, pinRow, optLabel, optCard, rcptLabel, receipt.el].forEach(e => e.style.display = cash ? '' : 'none');
      [cardLabel, cardBlk.el].forEach(e => e.style.display = m === 'card' ? '' : 'none');
...
function _buildKtxAccountCard() {
  const a = lsGetAccount('ktx');
  const card = makeGridCard();
  const memInp = makeTextInput(a.memberNo || '', 150, '회원번호'); memInp.inputMode = 'numeric';
  ...
  appendGridFull(card, '회원번호', memInp);
...
function buildTicketsPage() {
  ...
  function ticketCard(t, isRsv) {
    ...
    b.appendChild(route);
    const seatTxt = [t.seat_pick, t.seat_class ? `(${t.seat_class})` : ''].filter(Boolean).join(' ');
    if (seatTxt) b.appendChild(txt('div', 'tkc-meta seat', seatTxt));
    if (isRsv) {
      ...
      cancel.addEventListener('click', async () => {
        if (!await askConfirm(`${_dotDate(t.date)} ${t.dep}→${t.arr} ${_fmtHHMM(t.dep_time)} 예약을 취소할까요? 되돌릴 수 없습니다.`, cancel)) return;
  ...
  function load(force) {
    ...
    if (loading) return; if (!force && Date.now() - lastAt < 20000) return;
    loading = true;
...
function buildResultsPage() {
  ...
  function logBox(job) {
    ...
    const logEl = div('terminal-log'); logEl.style.height = '154px'; termWrap.appendChild(logEl);   // 기존 220px의 70%
  ...
  function makeCard(job) {
    ...
    c.append(c._head, c._trains, c._opt, c._meta, c._err, logBox(job));
    return c;
  }
  ...
  function fillTrains(host, j) {
    ...
    rows.forEach(t => {
      ...
      const won = j.won_train && String(j.won_train) === String(t.no);
      r.appendChild(txt('span', 'train-seat ' + (won ? 'ok' : 'watching'),
                        won ? '예매 성공' : (j.scheduled ? '대기' : '조회 중')));
      body.appendChild(r);
    });
  ...
  function fill(c, j) {
    const won = !!j.result;
    c._badge.textContent = j.running ? (j.scheduled ? '예약' : '감시 중') : (won ? '성공' : '종료');
    c._badge.className = 'job-badge ' + (j.running ? 'run' : (won ? 'ok' : 'idle'));
    c._name.textContent = _fmtJobTitle(j);
    // 감시가 끝나도 어떤 조건이었는지는 남아야 한다
    if (j.opt_text) c._optText = j.opt_text;
    const opt = c._optText ? `옵션: ${c._optText}` : '';
    c._opt.textContent = opt; c._opt.style.display = opt ? '' : 'none';
    const meta = j.last_check_at
      ? `조회: ${j.attempts}회 · 마지막 ${j.last_check_at}${j.running && j.next_delay_sec ? ` · 다음 ${j.next_delay_sec}초` : ''}`
      : (j.running ? '조회: 시작 준비 중' : (j.status_text || ''));
    c._meta.textContent = meta; c._meta.style.display = meta ? '' : 'none';
    const err = (!j.running && !won && j.last_error) ? String(j.last_error).slice(0, 90) : '';
    c._err.textContent = err; c._err.style.display = err ? '' : 'none';
    fillTrains(c._trains, j);
    const state = j.running ? 'run' : (won ? 'won' : 'stop');
    if (c._actState === state) return;
    c._actState = state;
    c._act.innerHTML = '';
    const mk = (label, cls, fn) => { const b = el('button', 'job-btn ' + cls); b.type = 'button'; b.textContent = label;
                                     b.addEventListener('click', () => { b.disabled = true; fn(); }); return b; };
    if (j.running) {
      c._act.appendChild(mk('중단', 'stop', () => api('POST', `/stop?mode=${encodeURIComponent(j.job)}`).finally(load)));
    } else if (won) {   // 성공한 작업은 삭제만
      c._act.appendChild(mk('삭제', 'del', () => api('DELETE', `/jobs?mode=${encodeURIComponent(j.job)}`).finally(load)));
    } else {
      c._act.append(
        mk('재개', 'go', () => api('POST', `/start?mode=${encodeURIComponent(j.job)}`, _jobBodies[j.job] || null)
          .then(r => { if (!r || !r.ok) notify('재개 실패: ' + ((r && r.error) || '예매 탭에서 다시 등록하세요')); })
          .catch(() => notify('재개 실패')).finally(load)),
        mk('삭제', 'del', () => api('DELETE', `/jobs?mode=${encodeURIComponent(j.job)}`).finally(load)));
    }
  }
  ...
  const _DEMO_JOBS = [
    ...
    { job: 'job2', running: false, label: '용산 → 광주송정', dep: '용산', arr: '광주송정', date: '20260912',
      ...
      opt_text: '후보 등록(일반실 우선)', won_train: '415',
  ...
  function render(jobs) {
    {
      ...
      jobs.forEach(j => {
        let c = cards[j.job];
        if (!c) { c = cards[j.job] = makeCard(j.job); listWrap.appendChild(c); }
        fill(c, j);
      });
    }
  }
...
async function _layoutFetch(trainType, carNo, updown) {
  const ud = updown === 'up' ? 'up' : 'down';
  const key = trainType + '|' + ud + (carNo ? '|' + carNo : '');
  if (_layoutCache[key]) return _layoutCache[key];
  const qs = `train_type=${encodeURIComponent(trainType)}&updown=${ud}` + (carNo ? `&car_no=${encodeURIComponent(carNo)}` : '');
...
function _candEditor(tpl, onSave, lockTrain) {
  ...
  function refreshRanks() {
    Object.keys(_seatCells).forEach(label => {
      ...
      cell.textContent = rk ? String(rk) : String(parseInt(label, 10));
  ...
  async function loadCars() {
    ...
    const r = await _layoutFetch(t.trainType, null, t.updown);
    carHost.innerHTML = '';
    if (!r || !r.ok) { carHost.appendChild(txt('div', 'train-list-msg', (r && r.error) || '편성을 불러오지 못했습니다')); return; }
    const chips = div('car-chips');
    (r.cars || []).forEach(c => {
      chips.appendChild(makeCarChip(c, parseInt(curCar, 10) === parseInt(c.car_no, 10),
        () => { curCar = c.car_no; drawCarChips(chips); drawMap(); }));
    });
    carHost.appendChild(wrapHScroll(chips));
    if (!curCar && (r.cars || []).length) curCar = r.cars[0].car_no;
    drawCarChips(chips); drawMap();
    // 나머지 호차 배치도 background로 채워둔다(다음 클릭부터는 즉시 표시)
    (async () => {
      for (const c of (r.cars || [])) {
        const no = parseInt(c.car_no, 10);
        if (_layoutCache[t.trainType + '|' + t.updown + '|' + no]) continue;
        try { await _layoutFetch(t.trainType, no, t.updown); } catch {}
  ...
  function drawCarChips(chips) {
    [...chips.children].forEach(ch => {
      const no = parseInt(ch.textContent, 10);
  ...
  async function drawMap() {
    ...
    const r = await _layoutFetch(t.trainType, parseInt(curCar, 10), t.updown);
    mapHost.innerHTML = '';
    ...
    renderSeatTable(mapHost, seats, {
      dirNote: `열차 진행 방향 →  ${t.updown === 'down' ? '하행' : '상행'}${r.train_no ? ' · ' + r.train_no + '편 기준' : ''}`,
  ...
  _editModal(tpl ? '후보 좌석 수정' : '후보 좌석 추가', (body) => {
    const c = makeGridCard();
    ...
    if (lockTrain !== undefined ? lockTrain : !!tpl) {   // 수정·기본행은 열차 고정(배치가 달라지면 순위가 깨진다)
      typeF.el.classList.add('locked');
      typeF.el.style.pointerEvents = 'none';
    }
    ...
    body.appendChild(div('sp10'));
    body.appendChild(txt('div', 'sub-label', '희망 순위 (위에서부터 1순위, 최대 10개)'));
    ...
    body.appendChild(div('sp10'));
    const c4 = makeGridCard();
    fbF = makeSegmentedField([['watch', '계속 감시'], ['auto', '조건 자동배정']], t.fallback === 'auto' ? 'auto' : 'watch');
    fbF.el.classList.add('seg-tight');
    appendStacked(c4, '후보가 전부 없을 때', fbF.el);
    body.appendChild(c4);
    drawSeats(); loadCars();
  }, () => {
    ...
    onSave({ id: t.id || (Date.now() + '' + Math.floor(Math.random() * 999)),
             ...
             trainType: t.trainType, updown: t.updown, fallback: fbF.getValue(), seats: t.seats });
  });
}
...
function buildSettingsPage() {
  ...
  panel.appendChild(div('sp10'));
  panel.appendChild(txt('div', 'group-header', '로그인'));
  const c = makeGridCard(); c.id = 'settings-card';
  panel.appendChild(c);
  panel.appendChild(div('sp18'));
  panel.appendChild(txt('div', 'group-header', '계정 · 결제'));
  const card = makeGridCard();
  function row(label, getVal, onClick) {
    const cell = div('grid-cell-full acct-row'); const val = txt('div', 'acct-state', '');
    cell.append(txt('div', 'grid-cell-label', label), val); cell.addEventListener('click', onClick);
    const refresh = () => { const v = getVal(); val.textContent = v || '정보 등록하기'; val.style.color = 'var(--blue)'; };
    refresh(); return { cell, refresh };
  }
  const r1 = row('KTX 계정', () => lsGetAccount('ktx').memberNo || '', () => _ktxAccountModal(() => r1.refresh()));
  const r2 = row('KTX 결제 수단', () => ktxPayConfigured() ? _payMethodLabel() : '', () => _ktxPayModal(() => r2.refresh()));
  card.append(r1.cell, div('grid-divider-h'), r2.cell);
  panel.appendChild(card);
  panel.appendChild(div('sp18'));
  // 내 정보(코레일) — 저장된 계정으로 모바일 API 조회(브라우저 불필요)
  panel.appendChild(txt('div', 'group-header', '내 정보'));
  ...
  loadMyInfo();
  panel.appendChild(miCard);
  loadGate();
  document.addEventListener('ktx-acct-sync', loadGate);
  panel.appendChild(div('sp18'));
  // 좌석 후보 — 예매 탭 '후보 등록'이 쓰는 우선순위 목록
  {
    const h = txt('div', 'group-header', '좌석 후보'); h.classList.add('gh-info');
    const i = txt('span', 'gh-i', 'i');
    i.title = '예매 탭 › 좌석 옵션 › 후보 등록에서 이 목록을 씁니다. 열차 종류와 진행 방향이 같으면 자동으로 골라집니다.';
    h.appendChild(i); panel.appendChild(h);
  }
  const candCard = makeGridCard(); panel.appendChild(candCard);
  // 기본 4종(KTX·KTX-산천 × 상·하행)은 등록 여부와 상관없이 항상 보여준다.
  const CAND_BASE = [['KTX', 'up'], ['KTX', 'down'], ['KTX-산천', 'up'], ['KTX-산천', 'down']];
  const _candName = (ty, ud) => `${ty} (${ud === 'up' ? '상행' : '하행'})`;
  function renderCand() {
    candCard.innerHTML = '';
    const list = _candLoad();
    const findT = (ty, ud) => list.find(x => (x.trainType || 'KTX') === ty && (x.updown || 'down') === ud);
    const extras = list.filter(t => !CAND_BASE.some(([ty, ud]) => (t.trainType || 'KTX') === ty && (t.updown || 'down') === ud))
                       .sort((a, b) => _candName(a.trainType, a.updown).localeCompare(_candName(b.trainType, b.updown), 'ko'));
    const rows = CAND_BASE.map(([ty, ud]) => ({ ty, ud, t: findT(ty, ud), base: true }))
      .concat(extras.map(t => ({ ty: t.trainType || 'KTX', ud: t.updown || 'down', t, base: false })));

    rows.forEach((r, i) => {
      if (i) candCard.appendChild(div('grid-divider-h'));
      const cell = div('grid-cell-full acct-row');
      cell.appendChild(txt('div', 'grid-cell-label', _candName(r.ty, r.ud)));
      const n = (r.t && (r.t.seats || []).length) || 0;
      cell.appendChild(n ? txt('div', 'acct-state', `선호 좌석 ${n}개`)
                         : txt('div', 'acct-state cand-empty', '선호 좌석을 등록하세요'));
      const open = () => _candEditor(r.t || { trainType: r.ty, updown: r.ud, fallback: 'watch', seats: [] }, o => {
        const l = _candLoad(); const k = l.findIndex(x => x.id === o.id);
        if (k >= 0) l[k] = o; else l.push(o);
        _candSave(l); renderCand();
      }, !!r.t || r.base);   // 기본 행은 열차가 고정
      cell.addEventListener('click', open);
      if (r.t) {
        const del = el('button', 'pbtn del cand-del'); del.type = 'button'; del.textContent = '삭제';
        del.addEventListener('click', async ev => {
          ev.stopPropagation();
          if (!await askConfirm(`'${_candName(r.ty, r.ud)}' 선호 좌석을 삭제할까요?`, del)) return;
          _candSave(_candLoad().filter(x => x.id !== r.t.id)); renderCand();   // 기본 4종은 행이 남는다
        });
        cell.appendChild(del);
      }
      candCard.appendChild(cell);
    });

    candCard.appendChild(div('grid-divider-h'));
    const add = div('grid-cell-stack');
    const b = el('button', 'preset-chip add cand-add'); b.type = 'button'; b.textContent = '열차 추가';
    b.addEventListener('click', () => _candEditor(null, o => { _candSave([..._candLoad(), o]); renderCand(); }));
    add.appendChild(b); candCard.appendChild(add);
  }
  renderCand();
  panel.appendChild(div('sp18'));
  ...
  _renderUpdateCard = (st) => {
    ...
    const newer = st.latest && st.latest !== st.current;
    updCard.appendChild(div('grid-divider-h'));
    const st2 = div('grid-cell-stack');
    if (st.needs_exe && newer) {
      st2.appendChild(txt('div', 'warn-line', `새 버전 ${st.latest}은 새 실행 파일이 필요합니다.\n아래 주소에서 내려받아 주세요.`));
      const a = document.createElement('a');
      a.href = 'https://github.com/jdent0228/trainer-releases/releases/latest';
      a.target = '_blank'; a.rel = 'noopener';
      a.textContent = 'trainer-releases 최신 릴리스';
      a.style.cssText = 'display:block;text-align:center;margin-top:8px;font-size:13px;color:var(--blue);font-weight:600';
      st2.appendChild(a);
    } else if (ready) {
      st2.appendChild(txt('div', 'note-line', `새 버전 ${st.ready} 준비 완료`));
      const b = el('button', 'btn-primary blue'); b.textContent = '업데이트 적용하고 다시 시작';
      b.style.marginTop = '10px';
      b.addEventListener('click', async () => {
        b.disabled = true; b.textContent = '적용하는 중…';
        const r = await api('POST', '/update/apply').catch(() => null);
        if (!r || !r.ok) {
          b.disabled = false; b.textContent = '업데이트 적용하고 다시 시작';
          notify((r && r.error) || '적용에 실패했습니다'); return;
        }
        b.textContent = '다시 시작하는 중…';
        setTimeout(() => location.reload(), 4000);
      });
      st2.appendChild(b);
    } else {
      const line = txt('div', 'hint', st.checking ? '확인하는 중…'
        : st.error ? `확인 실패 — ${st.error}`
        : '최신 버전입니다');
      line.style.textAlign = 'center'; st2.appendChild(line);
      const b = el('button', 'preset-chip add'); b.type = 'button'; b.textContent = '지금 확인';
      b.style.marginTop = '8px';
      b.addEventListener('click', async () => {
        b.disabled = true; b.textContent = '확인 중…';
        const r = await api('POST', '/update/check').catch(() => null);
        _renderUpdateCard(r || st);
        if (r && r.ok && r.ready && r.ready !== r.current) notify(`새 버전 ${r.ready}을 받았습니다`);
      });
      st2.appendChild(b);
    }
    updCard.appendChild(st2);
  };
  _pollUpdate();
  panel.appendChild(div('sp18'));
...
function _renderSettings() {
  ...
  if (!c) return;
  if (window.__DEMO__) {   // 데모: 서버 호출 없이 샘플 계정 표시
    c.innerHTML = '';
    _usageRow(c, '로그인 계정', 'demo@example.com');
    return;
  }
  fetch('/api/me').then(r => r.json()).then(d => {
    c.innerHTML = '';
    if (d.oauth && d.email) {
      _usageRow(c, '로그인 계정', d.email + (d.admin ? ' (관리자)' : d.approved ? '' : ' · 승인 대기 중'));
      c.appendChild(div('grid-divider-h'));
      const cell = div('grid-cell-stack');
      const act = div(''); act.style.cssText = 'display:flex;gap:8px;width:100%';
      if (d.admin) { const a = el('a', 'btn-secondary'); a.textContent = '관리자 페이지'; a.href = '/admin'; a.classList.add('u-linkbtn'); act.appendChild(a); }
      const lo = el('a', 'btn-secondary'); lo.textContent = '로그아웃'; lo.href = '/auth/logout'; lo.classList.add('u-linkbtn'); act.appendChild(lo);
      cell.appendChild(act); c.appendChild(cell);
    } else {
      _usageRow(c, 'Google 로그인', 'Google 로그인이 아직 설정되지 않았습니다. (GOOGLE_CLIENT_ID/SECRET 설정 시 활성화)');
    }
  }).catch(() => { c.innerHTML = ''; _usageRow(c, '계정', '정보를 불러올 수 없습니다.'); });
}
...
function _syncKeyOK(k) { return /^kor_(ktx|receipt)/.test(k); }
...
async function _syncPull() {
  ...
  try {
    ...
    if (j && j.ok && j.data && typeof j.data === 'object') {
      // 통째로 덮으면 이 기기의 PIN·카드번호가 날아간다 — 비밀 필드만 남기고 병합
      Object.keys(j.data).forEach(k => {
        if (!_syncKeyOK(k)) return;
        let v = j.data[k];
        try { v = _keepSecrets(v, localStorage.getItem(k)); } catch {}
...
function syncPush() {
  ...
  _syncT = setTimeout(() => {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      data[k] = _stripSecrets(localStorage.getItem(k));
    }
    fetch('/api/sync', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data }) }).catch(() => {});
  }, 800);
}
...
function showOnboarding(onDone) {
  ...
  const card = makeGridCard();
  const memInp = makeTextInput('', 150, '회원번호'); memInp.inputMode = 'numeric';
  memInp.addEventListener('input', () => { memInp.value = memInp.value.replace(/\D/g, ''); });
  ...
  const btn = makePrimaryButton('시작하기', 'blue', () => {
...
function _clusterCard(c) {
  ...
  row.appendChild(top);
  row.appendChild(txt('div', 'setup-cluster-sub', `${_WD_KO[c.weekday]}요일 ${c.actual_start}~${c.actual_end}`));
...
async function showSetup(onDone) {
  ...
  const listBox = _stageBody(bodyHost);
  ...
  // 조회된 열차를 몇 개만 흘려 보여준다(최신 순)
  const show = rides.slice(-7).reverse();
  show.forEach((r, i) => { r._last = i === show.length - 1; });
  for (const r of show) {
    // 마지막 줄이 들어갈 자리에 스피너가 있으면 줄이 늘고 스피너가 사라지며 화면이 두 번 움직인다.
    // 마지막 줄을 넣기 직전에 치워 자리를 그대로 넘겨준다.
    if (r._last) spin.remove();
  ...
  const clBox = _stageBody(bodyHost);
  const listCard = div('card'); listCard.style.opacity = '0'; clBox.appendChild(listCard);
  for (let i = 0; i < clusters.length; i++) {
    const row = _clusterCard(clusters[i]);
    listCard.appendChild(row);
    if (i === 0) { row.style.opacity = '1'; _fadeIn(listCard, SETUP_T.itemFade); }   // 흰 배경도 같이
    else _fadeIn(row, SETUP_T.itemFade);
...
function _boot() {
  ...
  installVersionWatch();
  _pollUpdate(); setInterval(_pollUpdate, 300000);   // 5분마다 업데이트 상태 확인
  refreshHeaderPill();
