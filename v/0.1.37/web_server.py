...
def _mail_body_rsv(p: Dict[str, Any], r: dict) -> str:
    d = str(r.get("date") or p.get("date_yyyymmdd") or "")
    dep_t, arr_t = str(r.get("dep_time") or "")[:4], str(r.get("arr_time") or "")[:4]
    def hm(x): return f"{x[:2]}:{x[2:]}" if len(x) == 4 else "-"
    lines = [
        f"{p['dep']} → {p['arr']}",
        f"{d[:4]}-{d[4:6]}-{d[6:]}  {hm(dep_t)} 출발 · {hm(arr_t)} 도착" if len(d) == 8 else "",
        f"{(r.get('train_type') or '').strip()} {(r.get('train_no') or '').strip()}".strip(),
        f"좌석 {r.get('seat_pick') or r.get('seat_label') or '-'}",
        f"{r.get('price'):,}원" if isinstance(r.get("price"), int) else "",
        "",
        f"결제기한 {r.get('buy_limit')[:8]} {r.get('buy_limit')[8:12]}" if r.get("buy_limit") else "",
        "",
        _BASE_URL,
    ]
...
async def runner_loop(st: WebState, p: Dict[str, Any], sid: Optional[str] = None, mode: str = "urgent"):
    ...
    CUR_MODE.set(mode)
    p["_owner"] = sid[2:] if (sid or "").startswith("u:") else ""   # 메일 알림 받을 사람
                ...
                for (train, seat, is_waiting, is_standing) in candidates:
                    ...
                    try:
                        ...
                        if is_waiting:
                            ...
                            r0 = st.result or {}
                            when = f"{p['date_yyyymmdd'][4:6]}/{p['date_yyyymmdd'][6:]}"
                            dt0 = str(r0.get("dep_time") or "")[:4]
                            await _mail_notify(p.get("_owner") or "",
                                f"[기차놀이] 예약 성공 · {p['dep']}→{p['arr']} {when}"
                                + (f" {dt0[:2]}:{dt0[2:]}" if len(dt0) == 4 else ""),
                                _mail_body_rsv(p, r0))
...
async def auth_callback(request: Request, code: str = "", state: str = "", error: str = ""):
    if state.startswith("cal-"):   # 캘린더 권한 동의 — 로그인이 아니라 토큰만 받아 저장한다
        _GCAL_STATE.pop(state, None)
        ...
        try:
            ...
            payload = _json.loads(_b64.urlsafe_b64decode(idt.split(".")[1] + "==").decode()) if idt else {}
            email = (payload.get("email") or "").lower() or (_session_email(request) or "")
            if email and tok.get("refresh_token"):
                _gcal_save(email, tok["refresh_token"])
            elif email:
                logger.warning("[캘린더] refresh_token 이 오지 않았습니다(이미 동의한 계정)")
        except Exception as e:
            logger.warning(f"[캘린더] 동의 처리 실패: {e!r}")
        return _Redirect("/#settings")
...
async def api_admin_mail_targets(request: Request):
    """공지 메일을 받을 사람 수(알림을 켠 사람만)."""
    if _session_email(request) not in _ADMIN_EMAILS:
        return {"ok": False, "error": "관리자만 가능합니다"}
    try:
        con = _mail_db()
        rows = [r[0] for r in con.execute("SELECT email FROM mailpref WHERE on_off=1").fetchall()]
        con.close()
    except Exception:
        rows = []
    return {"ok": True, "count": len(rows), "emails": rows, "sender": MAIL_FROM, "ready": bool(MAIL_ON)}


@app.post("/api/admin/mail")
async def api_admin_mail_send(request: Request):
    """주요 업데이트 등을 알림 켠 사람들에게 보낸다."""
    if _session_email(request) not in _ADMIN_EMAILS:
        return {"ok": False, "error": "관리자만 가능합니다"}
    if not MAIL_ON:
        return {"ok": False, "error": "보내는 계정이 설정되지 않았습니다"}
    try:
        body = await request.json()
    except Exception:
        return {"ok": False, "error": "bad json"}
    ...
    if not subject or not text:
        return {"ok": False, "error": "제목과 내용을 모두 적어 주세요"}
    try:
        con = _mail_db()
        rows = [r[0] for r in con.execute("SELECT email FROM mailpref WHERE on_off=1").fetchall()]
        con.close()
        ...
        rows = []
    if not rows:
        return {"ok": False, "error": "메일 알림을 켠 사람이 없습니다"}
    loop = asyncio.get_event_loop()
    sent = 0
    for to in rows:
        if await loop.run_in_executor(_executor, _mail_send_sync, to,
                                      subject if subject.startswith("[") else f"[기차놀이] {subject}",
                                      text + "\n\n" + _BASE_URL):
            sent += 1
    logger.info(f"[메일] 공지 발송 {sent}/{len(rows)}건 · {subject}")
...
async def api_mytickets(request: Request):
    ...
    email = _session_email(request) if _OAUTH_ON else None
    if email and res.get("ok") and _gcal_enabled(email):
        items = (res.get("reservations") or []) + (res.get("tickets") or [])
        asyncio.create_task(_gcal_sync(email, items))   # 화면을 붙잡지 않는다
    return res
...
# ── 메일 알림 ──────────────────────────────────────────────
# 예매 성공·결제 결과를 각자의 구글 로그인 이메일로 보낸다.
# 받는 쪽 권한은 필요 없다 — 보내는 계정(지메일 앱 비밀번호)만 있으면 된다(2026-09-10 지시).
MAIL_FROM = os.environ.get("MAIL_FROM", "").strip()
MAIL_APP_PW = os.environ.get("MAIL_APP_PW", "").strip()
MAIL_FROM_NAME = os.environ.get("MAIL_FROM_NAME", "기차놀이").strip()
MAIL_ON = bool(MAIL_FROM and MAIL_APP_PW)


def _mail_db():
    con = _sqlite3.connect(_SYNC_DB)
    con.execute("CREATE TABLE IF NOT EXISTS mailpref (email TEXT PRIMARY KEY, on_off INTEGER, updated TEXT)")
    return con


def _mail_enabled(email: str) -> bool:
    if not MAIL_ON or not email:
        return False
    try:
        con = _mail_db()
        row = con.execute("SELECT on_off FROM mailpref WHERE email=?", (email,)).fetchone()
        con.close()
...
def _mail_set(email: str, on: bool) -> None:
    try:
        con = _mail_db()
        con.execute("INSERT INTO mailpref(email,on_off,updated) VALUES(?,?,?) "
                    "ON CONFLICT(email) DO UPDATE SET on_off=excluded.on_off, updated=excluded.updated",
                    (email, 1 if on else 0, datetime.now().isoformat()))
        con.commit(); con.close()
        logger.info(f"[메일] {email} 알림 {'켜짐' if on else '꺼짐'}")
    except Exception as e:
        logger.warning(f"[메일] 설정 저장 실패: {e}")


def _mail_send_sync(to: str, subject: str, body: str) -> bool:
    import smtplib
    ...
    try:
        with smtplib.SMTP("smtp.gmail.com", 587, timeout=20) as sv:
            sv.starttls()
            sv.login(MAIL_FROM, MAIL_APP_PW)
            sv.send_message(msg)
        return True
    except Exception as e:
        logger.warning(f"[메일] 발송 실패({to}): {e}")
        return False


async def _mail_notify(email: str, subject: str, body: str) -> None:
    """켜 둔 사람에게만 보낸다. 실패해도 감시에는 영향을 주지 않는다."""
    if not _mail_enabled(email):
        return
    loop = asyncio.get_event_loop()
    ok = await loop.run_in_executor(_executor, _mail_send_sync, email, subject, body)
    if ok:
        logger.info(f"[메일] 발송 · {subject}")


@app.get("/api/mail")
async def api_mail(request: Request):
    email = _session_email(request) if _OAUTH_ON else None
    return {"ok": True, "enabled": _mail_enabled(email or ""), "available": bool(MAIL_ON and email),
            "reason": "" if MAIL_ON else "보내는 계정이 아직 설정되지 않았습니다"}


@app.post("/api/mail")
async def api_mail_set(request: Request):
    email = _session_email(request) if _OAUTH_ON else None
    if not email:
        return {"ok": False, "error": "로그인이 필요합니다"}
    if not MAIL_ON:
        return {"ok": False, "error": "보내는 계정이 아직 설정되지 않았습니다"}
    try:
        body = await request.json()
    except Exception:
        body = {}
    ...
    _mail_set(email, on)
    if on:
        await _mail_notify(email, "[기차놀이] 메일 알림을 켰습니다",
                           "예매·결제 결과를 이 주소로 보내 드립니다.\n\n끄려면 설정 › 추가 기능에서 바꾸세요.")
    return {"ok": True, "enabled": on}


# ── 구글 캘린더 연동 ───────────────────────────────────────
# 예매한 승차권을 그 사람 구글 캘린더에 자동으로 올린다. 캘린더 쓰기 권한은 무거우므로
# 로그인과 분리해, 설정에서 켠 사람만 따로 동의를 받는다(2026-09-10 지시).
GCAL_SCOPE = "https://www.googleapis.com/auth/calendar.events"
_GCAL_STATE: Dict[str, float] = {}   # 캘린더 동의 흐름 표시(state → 만든 시각)


def _gcal_db():
    con = _sqlite3.connect(_SYNC_DB)
    con.execute("CREATE TABLE IF NOT EXISTS gcal (email TEXT PRIMARY KEY, refresh_token TEXT, updated TEXT)")
    con.execute("CREATE TABLE IF NOT EXISTS gcal_events (email TEXT, key TEXT, event_id TEXT, updated REAL,"
                " PRIMARY KEY(email, key))")
    return con


def _gcal_enabled(email: str) -> bool:
    try:
        con = _gcal_db()
        row = con.execute("SELECT refresh_token FROM gcal WHERE email=?", (email,)).fetchone()
        con.close()
        return bool(row and row[0])
    except Exception:
        return False
...
def _gcal_save(email: str, refresh_token: str) -> None:
    try:
        con = _gcal_db()
        con.execute("INSERT INTO gcal(email,refresh_token,updated) VALUES(?,?,?) "
                    "ON CONFLICT(email) DO UPDATE SET refresh_token=excluded.refresh_token, updated=excluded.updated",
                    (email, refresh_token, datetime.now().isoformat()))
        con.commit(); con.close()
        logger.info(f"[캘린더] {email} 연동 켜짐")
    except Exception as e:
        logger.warning(f"[캘린더] 저장 실패: {e}")


def _gcal_off(email: str) -> None:
    try:
        con = _gcal_db()
        con.execute("DELETE FROM gcal WHERE email=?", (email,))
        con.execute("DELETE FROM gcal_events WHERE email=?", (email,))
        con.commit(); con.close()
        logger.info(f"[캘린더] {email} 연동 해제")
    except Exception as e:
        logger.warning(f"[캘린더] 해제 실패: {e}")


async def _gcal_access_token(email: str) -> str:
    try:
        con = _gcal_db()
        row = con.execute("SELECT refresh_token FROM gcal WHERE email=?", (email,)).fetchone()
        con.close()
    ...
    try:
        ...
        if not j.get("access_token"):
            logger.warning(f"[캘린더] 토큰 갱신 실패: {j.get('error')}")
            if j.get("error") == "invalid_grant":   # 사용자가 구글에서 권한을 뺐다
                _gcal_off(email)
            return ""
        return j["access_token"]
    except Exception as e:
        logger.warning(f"[캘린더] 토큰 요청 실패: {e}")
        return ""


def _gcal_title(t: dict) -> str:
    """[기차] 용산→광주송정 / 7호차 12A / KTX 405"""
    parts = [f"{t.get('dep') or '?'}→{t.get('arr') or '?'}"]
    seat = (t.get("seat_pick") or t.get("seat_label") or "").strip()
    if seat:
        parts.append(seat)
    train = f"{(t.get('train_type') or '').strip()} {(t.get('train_no') or '').strip()}".strip()
    if train:
        parts.append(train)
    return "[기차] " + " / ".join(parts)


def _gcal_when(t: dict):
    """(시작, 끝) RFC3339. 도착 시각이 없거나 자정을 넘으면 하루 더한다."""
    d = str(t.get("date") or "")
    dep, arr = str(t.get("dep_time") or "")[:4], str(t.get("arr_time") or "")[:4]
    if len(d) != 8 or len(dep) != 4:
        return None, None
    def iso(day, hhmm):
        return f"{day[:4]}-{day[4:6]}-{day[6:]}T{hhmm[:2]}:{hhmm[2:]}:00+09:00"
    start = iso(d, dep)
    if len(arr) == 4:
        day2 = d
        if arr < dep:   # 자정 넘김
            day2 = (datetime.strptime(d, "%Y%m%d") + timedelta(days=1)).strftime("%Y%m%d")
        return start, iso(day2, arr)
    end = datetime.strptime(d + dep, "%Y%m%d%H%M") + timedelta(hours=2)
    return start, end.strftime("%Y-%m-%dT%H:%M:00+09:00")


def _gcal_key(t: dict) -> str:
    return (t.get("rsv_id") or "").strip() or "|".join(
        str(t.get(k) or "") for k in ("d_wct", "d_dt", "d_sqno"))


async def _gcal_sync(email: str, tickets: list) -> None:
    """지금 가진 승차권을 캘린더에 맞춘다 — 없는 건 만들고, 사라진 건 지운다."""
    tok = await _gcal_access_token(email)
    if not tok:
        return
    try:
        con = _gcal_db()
        have = {k: v for k, v in con.execute(
            "SELECT key, event_id FROM gcal_events WHERE email=?", (email,)).fetchall()}
    except Exception:
        return
    want = {}
    for t in tickets or []:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            for k, t in want.items():
                if k in have:
                    continue
                start, end = _gcal_when(t)
                if not start:
                    continue
                body = {"summary": _gcal_title(t),
                        "start": {"dateTime": start, "timeZone": "Asia/Seoul"},
                        "end": {"dateTime": end, "timeZone": "Asia/Seoul"},
                        "source": {"title": "기차놀이", "url": _BASE_URL}}
                r = await c.post(url, headers=hdr, json=body)
                if r.status_code < 300:
                    ev = r.json().get("id")
                    con.execute("INSERT OR REPLACE INTO gcal_events(email,key,event_id,updated) VALUES(?,?,?,?)",
                                (email, k, ev, time.time()))
                    added += 1
                else:
                    logger.warning(f"[캘린더] 등록 실패 {r.status_code} · {r.text[:120]}")
            for k, ev in have.items():
                if k in want or not ev:
                    continue
                await c.delete(f"{url}/{ev}", headers=hdr)   # 취소·사용 완료된 표는 일정도 지운다
                con.execute("DELETE FROM gcal_events WHERE email=? AND key=?", (email, k))
                removed += 1
        con.commit()
    except Exception as e:
        logger.warning(f"[캘린더] 동기화 실패: {e}")
    finally:
        try: con.close()
        except Exception: pass
    if added or removed:
        logger.info(f"[캘린더] 일정 {added}건 등록 · {removed}건 삭제")


@app.get("/api/gcal")
async def api_gcal(request: Request):
    email = _session_email(request) if _OAUTH_ON else None
    if not email:
        return {"ok": True, "enabled": False, "available": False}
    return {"ok": True, "enabled": _gcal_enabled(email), "available": True}


@app.post("/api/gcal/off")
async def api_gcal_off(request: Request):
    email = _session_email(request) if _OAUTH_ON else None
    if not email:
        return {"ok": False, "error": "로그인이 필요합니다"}
    _gcal_off(email)
...
async def auth_calendar(request: Request):
    """캘린더 쓰기 권한만 따로 받는다(로그인 흐름과 같은 redirect_uri 를 쓰되 state 로 구분)."""
    if not _OAUTH_ON:
        return _HTML(_page("설정 필요", "<p>구글 로그인이 설정되지 않았습니다.</p>"), status_code=503)
    st = "cal-" + _secrets.token_urlsafe(16)
    _GCAL_STATE[st] = time.time()
    for k, v in list(_GCAL_STATE.items()):
        if time.time() - v > 600:
            _GCAL_STATE.pop(k, None)
    params = {"client_id": _GG_ID, "redirect_uri": _redirect_uri(request), "response_type": "code",
              "scope": "openid email " + GCAL_SCOPE, "state": st,
              "access_type": "offline", "prompt": "consent", "include_granted_scopes": "true"}
    return _Redirect("https://accounts.google.com/o/oauth2/v2/auth?" + _urlparse.urlencode(params))


# ── 로그 문구 다듬기(관리자) ────────────────────────────────
# 실시간 로그에 나오는 문구를 한 화면에서 보고 원하는 표현으로 적어 두는 곳.
# 여기 적힌 값을 보고 코드의 문구를 바꾼다(이 파일이 곧 작업 지시서다).
_LOGTEXT_FILE = data_path(".logtext.json")
_LOGTEXT_ITEMS = [
    ("시작 · 종료", [
        ("start", "감시 시작 · 수서→광주송정 10/12 · 1명 · 10초 · 605편 · 조회 06:00~06:54", ""),
        ("sched_wait", "예약 대기 — 09/12 07:00 시작 예정", "07:00 시작 예정"),
        ("burst", "집중 감시 — 5초 주기, 10분", "집중 감시 5초 · 10분"),
        ("cancel", "예약 시작 취소됨", "취소됨"),
        ("stopped", "감시 중단 · 조회 1204회", "중단 · 1204회"),
        ("done_ok", "감시 종료 · 예약 성공 · 조회 87회", "종료 · 예약 성공 · 87회"),
        ("done_etc", "감시 종료 · 중단됨 · 조회 87회 · …", "종료 · 중단됨 · 87회"),
        ("oneshot", "1회 시도 실패 — 종료", "실패 · 종료"),
        ("restore", "이전 감시 3건 — 결과 탭에서 직접 재개하세요", "이전 감시 3건 · 재개 대기"),
    ]),
    ("로그인 · 세션", [
        ("login_ok", "로그인 성공 · 정영우", "로그인 · 정영우"),
        ("sess", "세션 갱신 (#90)", "세션 갱신"),
        ("sess_fail", "세션 갱신 실패: …", "세션 갱신 실패"),
        ("sess_exp", "세션 만료(P058) — 재로그인 후 재시도", "세션 만료 · 재로그인"),
        ("login_fail", "로그인 실패 · … + 감시 종료 · 로그인 실패 (두 줄)", "로그인 실패 · 종료 (한 줄)"),
    ]),
    ("조회", [
        ("found", "10편 조회 (14:09~18:22)", "10편 · 14:09~18:22"),
        ("cand_n", "#20 후보 3개", "후보 3개"),
        ("cand_row", "후보|18:30~21:05|KTX 063", "후보 KTX 063 18:30"),
        ("excl", "다른 역 25건 제외 · 용산→광주, …", "다른 역 25건 제외"),
        ("q_fail", "조회 실패 · 발매 개시 준비 중입니다. (WRD000052)", "조회 실패 · 발매 개시 전"),
        ("open_wait", "예매 개시 09/12 07:00 — 3분 전까지 조회를 멈춥니다", "07:00 개시 · 3분 전까지 대기"),
        ("open_acc", "#N 예매 개시 전 — 대기 (누적 12)", "(삭제)"),
        ("hol", "#N 명절 차단(ERR299929) — 잔여석 개시까지 대기 (누적 30)", "명절 차단 · 개시까지 대기"),
        ("hol_off", "#N 명절 차단 해제 — 정상 감시", "명절 차단 해제"),
    ]),
    ("좌석 고르기", [
        ("rank_ok", "후보 1순위, 2순위 확보 → 7호차 12A·12B", "1·2순위 확보 · 7호차 12A·12B"),
        ("rank_part_auto", "후보 2/3석뿐 — 조건 자동 선택", "후보 2/3석 · 자동 선택"),
        ("rank_part_wait", "후보 1/3석 — 계속 감시", "후보 1/3석 · 대기"),
        ("no_tpl", "605편(편성미상) 선호 좌석 없음 — 다음 열차", "605편 선호 좌석 없음"),
        ("auto_pick", "자동 선택 → 7호차 12A", "자동 선택 · 7호차 12A"),
        ("strict_skip", "빈자리 8개 — 선호 옵션과 달라 건너뜀", "빈자리 8개 · 조건 불일치"),
        ("loose", "조건 맞는 자리 없음 — 일반 예약", "조건 불일치 · 일반 예약"),
        ("map_fail", "좌석맵 실패(…) — 이번 사이클 건너뜀", "좌석맵 실패 · 건너뜀"),
        ("band", "605편 편성(11~18호차)에 맞춰 후보 좌석 호차 5건 옮김", "11~18호차 편성 · 5건 조정"),
    ]),
    ("예약 · 결제", [
        ("fixed_ok", "지정 좌석 예약 성공 · 7호차 12A", "예약 성공 · 7호차 12A"),
        ("fixed_fb", "지정 좌석 실패(…) — 아무 좌석으로 폴백", "지정 실패 · 일반 예약"),
        ("rsv_ok", "예약 성공 · (예약 정보 전체)", "예약 성공 · KTX 063 7호차 12A · 59,800원"),
        ("seat_assigned", "배정 좌석 · 7호차 12A", "(삭제 — 예약 성공과 중복)"),
        ("ncard", "N카드 할인 요청 · ****1234 · 35,400원", "N카드 · 35,400원"),
        ("rsv_fail", "예약 실패 · …", ""),
        ("pay_start", "[자동결제] 시작 (train_no=63, dry_run=False)", "결제 시작"),
        ("pay_toss", "[토스결제] 시작 / [카드결제] 시작", "(삭제 — 자동결제와 중복)"),
        ("pay_done", "[자동결제] 완료 ✅", "결제 완료"),
        ("pay_fail", "[자동결제] 실패: …", "결제 실패 · 사유"),
        ("pay_queue", "[자동결제] 같은 계정 결제 진행 중 — 순번 대기", "결제 대기"),
    ]),
]


def _logtext_load() -> dict:
    try:
        with open(_LOGTEXT_FILE, encoding="utf-8") as f:
...
@app.get("/api/logtext")
async def api_logtext_get(request: Request):
    if _session_email(request) not in _ADMIN_EMAILS and not _is_loopback(request):
        return {"ok": False, "error": "관리자만"}
    return {"ok": True, "groups": [{"title": t, "items": [
        {"id": i, "cur": c, "sug": g} for i, c, g in items]} for t, items in _LOGTEXT_ITEMS],
        "saved": _logtext_load()}


@app.post("/api/logtext")
async def api_logtext_set(request: Request):
    if _session_email(request) not in _ADMIN_EMAILS and not _is_loopback(request):
        return {"ok": False, "error": "관리자만"}
    try:
        body = await request.json()
    ...
    if not isinstance(data, dict):
        return {"ok": False, "error": "no data"}
    try:
        with open(_LOGTEXT_FILE, "w", encoding="utf-8") as f:
            _json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception as e:
        return {"ok": False, "error": str(e)}
    logger.info(f"[로그문구] {len(data)}건 저장")
    return {"ok": True, "saved": len(data)}


@app.get("/logtext")
async def logtext_page(request: Request):
    if _session_email(request) not in _ADMIN_EMAILS and not _is_loopback(request):
        return _HTML(_page("권한 없음", "<p>관리자만 볼 수 있습니다.</p>"), status_code=403)
    html = """<!doctype html><html lang=ko><head><meta charset=utf-8>
<meta name=viewport content="width=device-width,initial-scale=1"><title>로그 문구 다듬기</title><style>
 :root{--line:#E5E8EB;--muted:#8B95A1;--blue:#3182F6}
 *{box-sizing:border-box} body{margin:0;background:#F2F4F6;color:#191F28;
 ...
 h1{margin:0;font-size:16px} .sub{color:var(--muted);font-size:12.5px}
 .save{margin-left:auto;border:none;background:var(--blue);color:#fff;border-radius:9px;padding:8px 16px;
   font-size:13.5px;font-weight:700;cursor:pointer} .save:disabled{opacity:.5}
 .ok{color:#0CA678;font-size:12.5px;font-weight:700}
 main{padding:14px 16px 60px;max-width:1100px;margin:0 auto}
 h2{font-size:13px;color:var(--muted);margin:18px 0 8px;font-weight:700}
 .card{background:#fff;border-radius:14px;overflow:hidden}
 .r{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:10px 14px;border-bottom:1px solid var(--line);align-items:center}
 .r:last-child{border-bottom:none}
 .cur{font-size:13px;color:#4E5968;word-break:break-all}
 .id{display:block;font-size:10.5px;color:#B0B8C1;margin-bottom:2px}
 input{width:100%;border:1px solid var(--line);border-radius:9px;padding:8px 10px;font-size:13.5px;font-family:inherit}
 input:focus{outline:none;border-color:var(--blue)}
 input::placeholder{color:#C4CBD4}
 @media(max-width:720px){.r{grid-template-columns:1fr;gap:6px}}
</style></head><body>
<header><div><h1>실시간 로그 문구 다듬기</h1>
<div class=sub>왼쪽이 지금 나오는 문구입니다. 오른쪽에 원하는 표현을 적어 주세요. 비워 두면 그대로 둡니다.</div></div>
<span id=msg class=ok></span><button class=save id=save>저장</button></header>
<main id=main>불러오는 중…</main>
<script>
var saved={};
function esc(t){var d=document.createElement('div');d.textContent=t;return d.innerHTML}
fetch('/api/logtext').then(function(r){return r.json()}).then(function(d){
 if(!d.ok){document.getElementById('main').textContent=d.error||'오류';return}
 saved=d.saved||{};
 var h='';
 d.groups.forEach(function(g){
  h+='<h2>'+esc(g.title)+'</h2><div class=card>';
  g.items.forEach(function(it){
   var v=saved[it.id]!==undefined?saved[it.id]:'';
   h+='<div class=r><div class=cur><span class=id>'+esc(it.id)+'</span>'+esc(it.cur)+'</div>'+
      '<input data-id="'+esc(it.id)+'" value="'+esc(v)+'" placeholder="'+esc(it.sug||'그대로 둠')+'"></div>';
  });
  h+='</div>';
 });
...
document.getElementById('save').onclick=function(){
 ...
 var data={};
 [].forEach.call(document.querySelectorAll('input[data-id]'),function(i){
  var v=i.value.trim(); if(v) data[i.dataset.id]=v; });
 fetch('/api/logtext',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({data:data})})
  .then(function(r){return r.json()}).then(function(d){
   document.getElementById('msg').textContent=d.ok?('저장됨 · '+d.saved+'건'):(d.error||'실패');
   setTimeout(function(){document.getElementById('msg').textContent=''},3000); b.disabled=false;});
};
</script></body></html>"""
    return _HTML(html, headers={"Cache-Control": "no-store"})
