...
function _buildModePanel(panel, cfg) {
  ...
  etcCell.addEventListener('click', () => {
    // 끈 상태에서는 설정할 것이 없다 — 토글만 눌리게 한다
    if (seatMode() === 'cand' && !candFallback) return;
    openEtcSheet();
  });
  function renderEtc() {
    ...
    etcCell.style.cursor = off ? 'default' : 'pointer';
...
function buildSettingsPage() {
  ...
  // 추가 기능 — 각 행 오른쪽에 토글 스위치. 행을 누르면 설명 팝업이 열린다.
  panel.appendChild(div('sp18'));
  panel.appendChild(txt('div', 'group-header', '추가 기능'));
  const extCard = makeGridCard();
  function extRow(label, onToggle, onExplain) {
    const cell = div('grid-cell-full acct-row');
    const sw = el('button', 'switch'); sw.type = 'button'; sw.setAttribute('role', 'switch');
    sw.innerHTML = '<span class="switch-knob"></span>';
    sw.addEventListener('click', ev => { ev.stopPropagation(); onToggle(sw.classList.contains('on')); });
    cell.append(txt('div', 'grid-cell-label', label), sw);
    cell.addEventListener('click', () => onExplain());
    extCard.appendChild(cell);
    return { cell, sw, set(on) { sw.classList.toggle('on', !!on); sw.setAttribute('aria-checked', on ? 'true' : 'false'); } };
  }

  let gcOn = false, mlOn = false, mlReady = true, mlWhy = '';
  const gcRow = extRow('구글 캘린더 연동',
    (on) => { if (on) gcalOff(); else _gcalSheet(); },
    () => _gcalSheet());
  extCard.appendChild(div('grid-divider-h'));
  const mlRow = extRow('메일 알림',
    (on) => mailSet(!on),
    () => _mailSheet());
  panel.appendChild(extCard);

  function paintGcal() {
    api('GET', '/gcal').then(d => { gcOn = !!(d && d.enabled); gcRow.set(gcOn); }).catch(() => gcRow.set(false));
  }
  function paintMail() {
    api('GET', '/mail').then(d => {
      mlOn = !!(d && d.enabled); mlReady = !!(d && d.available); mlWhy = (d && d.reason) || '';
      mlRow.set(mlOn);
    }).catch(() => mlRow.set(false));
  }
  paintGcal(); paintMail();

  async function gcalOff() {
    if (!(await askConfirm('구글 캘린더 연동을 끌까요? 이미 올린 일정은 그대로 남습니다.', gcRow.sw))) return;
    await api('POST', '/gcal/off').catch(() => null);
    paintGcal(); notify('구글 캘린더 연동을 껐습니다.');
  }
  async function mailSet(on) {
    if (on && !mlReady) { notify(mlWhy || '아직 준비 중입니다.', mlRow.sw); return; }
    const r = await api('POST', '/mail', { on }).catch(() => null);
    if (!r || !r.ok) { notify((r && r.error) || '바꾸지 못했습니다.', mlRow.sw); return; }
    paintMail();
    notify(r.enabled ? '메일 알림을 켰습니다. 확인 메일을 보냈습니다.' : '메일 알림을 껐습니다.');
  }
  function _sheet(title, lines, btnText, onGo, danger) {
    const ov = div('picker-overlay'); const sheet = div('picker-sheet'); sheet.classList.add('u-sheet-pad');
    sheet.appendChild(txt('div', 'wheel-title', title));
    const box = div('grid-cell-stack');
    lines.forEach(t => box.appendChild(txt('div', 'note-line' + (t.muted ? ' muted' : ''), t.text || t)));
    ...
    if (btnText) {
      const go = el('button', 'btn-primary'); go.textContent = btnText;
      if (danger) go.classList.add('btn-ghost', 'btn-danger');
      go.addEventListener('click', () => { document.body.removeChild(ov); onGo(); });
      bar.appendChild(go);
    }
    sheet.appendChild(bar); ov.appendChild(sheet);
    ov.addEventListener('click', e => { if (e.target === ov) document.body.removeChild(ov); });
    document.body.appendChild(ov);
  }
  function _gcalSheet() {
    if (gcOn) { _sheet('구글 캘린더 연동', [{ text: '지금 연동되어 있습니다.' }], '연동 해제', gcalOff, true); return; }
    _sheet('구글 캘린더 연동', [
      { text: '승차권 정보가 구글 캘린더와 연동됩니다.\n취소 및 변경도 반영됩니다.' },
      { text: '구글 캘린더 쓰기 권한이 필요하며, 용도 외에는 사용하지 않습니다.', muted: true },
    ], '연동하기', () => { location.href = '/auth/calendar'; });
  }
  function _mailSheet() {
    const lines = [{ text: '예매에 성공하면 로그인한 구글 메일로 알려 드립니다.\n열차·좌석·결제기한이 함께 담깁니다.' }];
    if (!mlReady) lines.push({ text: mlWhy || '아직 준비 중입니다.', muted: true });
    _sheet('메일 알림', lines, mlOn ? '끄기' : '켜기', () => mailSet(!mlOn), mlOn);
  }
