...
function renderSeatTable(host, seats, opts) {
  ...
  if (cols.every(c => !c)) { _renderSeatNumGrid(host, ordered, o); return; }
...
function _buildModePanel(panel, cfg) {
  ...
  const hourF = makeSheetPicker(Array.from({ length: 24 }, (_, h) => [h, String(h).padStart(2, '0') + ':00']), startHour,
    v => { startHour = +v; hourSpan = 1; _showN = 10; selTrains = []; _flashList(); renderTrainSel(); save();
           // 시간대가 바뀌면 그 구간을 새로 조회해 매진/가능 상태를 최신으로(캐시 무시, ~1콜)
           if (isMulti()) _ensureWindow(_trainKey(), timeRange.getStart(), timeRange.getEnd()).then(() => renderTrainSel()); });
  ...
  function _renderTrainSel() {
    ...
    if (!_winFresh && _trainFetchKey !== _winKey) {
      ...
      _ensureWindow(key, a, '')
        .then(r => { if (r && r.ok === false) _trainErr = r.error || ''; })
        .catch(() => {})
  ...
  { // 선호 좌석 모드: 선호 옵션이 '예매 실패 시 폴백 조건'임을 알린다
    if (seatCard.children.length) seatCard.appendChild(div('grid-divider-h'));
    const _nd = seatCard.lastElementChild && seatCard.lastElementChild.classList.contains('grid-divider-h') ? seatCard.lastElementChild : null;
    const noteCell = div('grid-cell-stack');
    noteCell.appendChild(txt('div', 'note-line', '선호 좌석 예매 실패 시, 아래 조건으로 예매합니다'));
    seatCard.appendChild(noteCell);
    rowEls['candNote'] = [_nd, noteCell].filter(Boolean);
  }
  ...
  function syncSections() {
    ...
    show(rowEls, 'candNote', seatMode() === 'cand');
  ...
  function getStartBody() {
    ...
    if (isManual()) {
      ...
      const nos = (fixed && fixed.train) ? [fixed.train.no] : selTrains;
      ...
      base.cand_fallback = first.fallback === 'auto' ? 'auto' : 'watch';
...
function buildResultsPage() {
  ...
  function fill(c, j) {
    ...
    if (j.running) {
      ...
      c._act.append(
        mk(_ICON_PLAY, 'go', '재개', () => api('POST', `/start?mode=${encodeURIComponent(j.job)}`, _jobBodies[j.job] || null)
  ...
  const _DEMO_JOBS = [
    ...
    { job: 'job2', running: false, label: '용산 → 광주송정', dep: '용산', arr: '광주송정', date: '20260912',
      ...
      opt_text: '선호 좌석(일반실 우선)', won_train: '415',
...
function _mountCallLog() {
  ...
  head.append(txt('span', 'calllog-title', '코레일 API 호출'), (() => {
    const x = el('button', 'calllog-x'); x.type = 'button'; x.textContent = '×';
    x.addEventListener('click', () => setOn(false)); return x; })());
  const body = div('calllog-body'); panel.append(head, body); document.body.appendChild(panel);
  let on = false, timer = null, after = 0;
  function row(c) {
    const r = div('calllog-row');
    r.append(txt('span', 'cl-t', c.t),
             txt('span', 'cl-purpose ' + (c.host === 'web' ? 'web' : 'mob'), c.purpose),
             txt('span', 'cl-ep', c.ep),
             txt('span', 'cl-params', c.params || ''));
    r.title = `${c.method} ${c.url}` + (c.params ? '\n' + c.params : '');
    return r;
  }
  function poll() {
    api('GET', '/calllog?after=' + after).then(d => {
      if (!d || !d.ok) return;
      (d.calls || []).forEach(c => { after = Math.max(after, c.n); body.appendChild(row(c)); });
