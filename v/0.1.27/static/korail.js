...
function buildSettingsPage() {
  ...
  function row(label, getVal, onClick, btn) {
    const cell = div('grid-cell-full acct-row');
    const val = txt('div', 'acct-state', '');
    ...
    const refresh = () => {
      ...
      val.style.color = v ? '' : 'var(--blue)';
    };
    refresh(); return { cell, refresh };
  }
  // 구글 로그인 계정 — /api/me 로 채운다
  const me = { email: '', admin: false };
  const r0 = row('로그인 계정', () => me.email, null, { text: '로그아웃', onClick: async (b) => {
  ...
  const r1 = row('KTX 계정', () => lsGetAccount('ktx').memberNo || '', () => _ktxAccountModal(() => r1.refresh()),
    { text: '로그아웃', onClick: async (b) => {
      if (!lsGetAccount('ktx').memberNo) { notify('등록된 코레일 계정이 없습니다.', b); return; }
      if (!(await askConfirm('KTX 계정 정보를 지울까요? 연결된 기기에서도 다시 로그인해야 합니다.', b))) return;
      ktxLogout();   // 지우고 최초 로그인 화면으로
    } });
  ...
  // 마지막 행: 코레일에 등록된 내 정보 자세히 보기
  const detCell = div('grid-cell-full acct-row');
  const detVal = txt('div', 'acct-state', '›');
  detCell.append(txt('div', 'grid-cell-label', 'KTX 계정 정보 자세히보기'), detVal);
  detCell.addEventListener('click', () => _ktxInfoSheet());
  card.append(div('grid-divider-h'), detCell);
  _renderSettings = () => {   // /api/me 결과로 로그인 계정 행을 채운다
    ...
    fetch('/api/me').then(r => r.json()).then(d => {
      ...
      r0.refresh();
      if (me.admin && !card.querySelector('.acct-admin')) {   // 관리자에게만 보이는 이동 링크
        const a = div('grid-cell-full acct-row acct-admin');
        const lnk = el('a', 'acct-mini-btn'); lnk.textContent = '열기'; lnk.href = '/admin';
        const rt = div(''); rt.classList.add('u-row8'); rt.appendChild(lnk);
        a.append(txt('div', 'grid-cell-label', '관리자 페이지'), rt);
  ...
  const prefWrap = div('etc-wrap'); prefWrap.appendChild(prefVal);
  ...
  function _prefSheet() {
    const ov = div('picker-overlay'); const sheet = div('picker-sheet');
    sheet.appendChild(txt('div', 'wheel-title', '선호 옵션 설정'));
    ...
    const chipRow = (label, opts, key) => {
      ...
      const cell = div('grid-cell-full'); cell.classList.add('chip-row');
      const chip = makeChipGroup(opts, () => lsGetSeatPref()[key], v => { lsSetSeatPref({ [key]: v }); renderPrefSummary(); });
      const act = div('grid-cell-action'); act.appendChild(chip.el);
      cell.append(txt('div', 'grid-cell-label', label), act);
    ...
    const toggleRow = (label, key, note) => {
      ...
      cb.addEventListener('change', () => { lsSetSeatPref({ [key]: cb.checked }); renderPrefSummary(); });
      const act = div('grid-cell-action'); act.appendChild(cb);
      cell.append(txt('div', 'grid-cell-label', label), act);
      listCard.appendChild(cell);
      if (note) listCard.appendChild(txt('div', 'etc-note', note));
    };
    chipRow('좌석등급', _SEAT_OPTS, 'seatClass');
    chipRow('창측 / 내측', _SIDE_OPTS, 'seatSide');
    chipRow('진행 방향', _DIR_OPTS, 'seatDir');
    toggleRow('예약대기 허용', 'allowWaiting', '좌석이 없으면 예약대기로 걸어 둡니다(자동결제는 안 됩니다).');
    toggleRow('입석 허용', 'allowStanding', '좌석이 없으면 입석으로 잡습니다.');
    sheet.appendChild(listCard);
    const bar = div('preset-actions'); bar.style.marginTop = '12px';
  ...
  _ktxInfoSheet = () => {
    ...
    const bar = div('preset-actions'); bar.style.marginTop = '12px';
    const close = el('button', 'btn-primary'); close.textContent = '닫기'; close.classList.add('btn-ghost');
    close.addEventListener('click', () => { document.body.removeChild(ov); panel.appendChild(miCard); miCard.style.display = 'none'; });
    bar.appendChild(close); sheet.appendChild(bar);
    ov.appendChild(sheet);
