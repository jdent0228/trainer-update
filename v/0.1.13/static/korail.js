...
function _seatGroupsInit() {
  return api('GET', '/seatgroups').then(d => { if (d && d.ok) _byTrain = d.by_train || {}; }).catch(() => {});
}
function _groupIdOf(trainType, trainNo) { return (_byTrain || {})[`${trainType}|${trainNo}`] || ''; }

// 병합 그룹(1~8호차 + 중련 11~18호차)에서 그 열차가 실제로 쓰는 호차 번호로 옮긴다
let _groupsById = null;
function _bandOfTrain(gid, trainNo) {
  const g = (_groupsById || {})[gid];
  if (!g || !g.trains_by_band) return '';
  for (const [b, list] of Object.entries(g.trains_by_band)) if (list.includes(String(trainNo))) return b;
  return g.canon_band || '';
}
function _carForTrain(gid, trainNo, carNo) {
  const g = (_groupsById || {})[gid];
  if (!g || (g.bands || []).length < 2) return carNo;
  const band = _bandOfTrain(gid, trainNo);
  const n = parseInt(carNo, 10);
  if (!n) return carNo;
  if (band === '11-18' && n <= 8) return String(n + 10);
  if (band === '1-8' && n >= 11) return String(n - 10);
  return carNo;
}
function _groupLabel(g) {
  return `${g.name || g.train_type} (${g.updown === 'up' ? '상행' : '하행'})`;
}
...
function _buildModePanel(panel, cfg) {
  ...
  addRow(seatCard, 'cand', '후보 좌석', candList,
         '설정 › 좌석 후보에서 등록한 우선순위대로 시도합니다. 열차마다 좌석 배치에 맞는 후보가 자동으로 붙습니다.');
  ...
  function _candTemplateFor(trainNo) {
    ...
    const gid = _groupIdOf(ty, trainNo);
    const list = _candLoad();
    if (gid) { const m = list.find(x => x.groupId === gid); if (m) return m; }
    // 아직 조사되지 않은 열차 — 예전 방식(종류+방향)으로 한 번 더 찾아본다
    const ud = parseInt(trainNo, 10) % 2 ? 'down' : 'up';
    return list.find(x => !x.groupId && x.trainType === ty && (!x.updown || x.updown === ud)) || null;
  }

  const candPv = { el: candList, refresh: () => {
    candList.innerHTML = '';
    const nos = (fixed && fixed.train) ? [fixed.train.no] : selTrains.slice();
    if (!nos.length) { candList.appendChild(txt('div', 'cand-auto-empty', '열차를 먼저 선택하세요')); return; }
    const rows = _tcList(_trainKey(), timeRange.getStart(), timeRange.getEnd()) || [];
    nos.forEach(no => {
      const tr = rows.find(x => x.no === no) || {};
      const t = _candTemplateFor(no);
      const line = div('cand-auto-row');
      line.appendChild(txt('span', 'cand-auto-train', _trainLabel(tr.no ? tr : { no, type: tr.type })));
      const seats = (t && t.seats) || [];
      const gid = _groupIdOf(tr.type || '', no);
      line.appendChild(seats.length
        ? txt('span', 'cand-auto-seats',
              seats.map(x => _seatLabel({ ...x, car_no: _carForTrain(gid, no, x.car_no) })).join(', '))
        : txt('span', 'cand-auto-none', '등록 안 함'));
      candList.appendChild(line);
    });
  } };
  ...
  function _loadSkeleton(sync) {
    ...
    const gid = _groupIdOf(ty, f.train && f.train.no);
  ...
  function _prefetchLayouts(cars) {
    ...
    (async () => {
      for (const c of cars) {
        ...
        try { await _layoutFetch(ty, no, ud, _groupIdOf(ty, f.train && f.train.no)); } catch {}
...
function _payMethodLabel() { const m = _payMethodOf(lsGetPay()); const hit = _PAY_METHODS.find(x => x[0] === m); return hit ? hit[1] : '토스페이'; }
...
function _ktxPayModal(onSaved) {   // 결제 정보 모달 — 저장은 계정 레코드의 pay로(lsSetPay)
  ...
  _editModal('결제 정보', (body) => {
    ...
    pinInp.addEventListener('input', () => mNote.paint());
    // ①-b 카드 정보 (신용카드 전용)
    cardSec = div(''); body.appendChild(cardSec);
    ...
    // ② 발권 · 할인 (간편현금결제 전용)
    optSec = div(''); body.appendChild(optSec);
    ...
    optSec.append(optCard, div('sp10'));
    // ③ 현금영수증 (간편현금결제 전용)
    rcptSec = div(''); body.appendChild(rcptSec);
  ...
  async function loadCars() {
    ...
    (async () => {
      for (const c of (r.cars || [])) {
        ...
        try { await _layoutFetch(t.trainType, no, t.updown, t.groupId); } catch {}
...
function buildSettingsPage() {
  ...
  // 노선은 구분하지 않는다(중요한 건 좌석맵). 상세는 이름 옆 i 아이콘에 담는다.
  function renderCand() {
    candWrap.innerHTML = '';
    candWrap.appendChild(loadingBox('불러오는 중…'));
    _seatGroups().then(groups => {
      candWrap.innerHTML = '';
      const card = makeGridCard();
      if (!groups.length) {
      ...
      const list = _candLoad();
      // 기본 목록은 서버가 이름순(ㄱㄴㄷ)으로 정렬해 준 순서를 그대로 쓴다.
      // 목록에 없는(사용자가 따로 만든) 항목은 그 아래에 만든 순서대로 붙는다.
      const known = new Set(groups.map(g => g.id));
      const extra = list.filter(x => x.groupId && !known.has(x.groupId));
      groups.concat(extra.map(x => ({
        id: x.groupId, name: x.trainType || '기타', train_type: x.trainType || '',
        updown: x.updown || 'down', trains: [], cars_label: '', seats: 0, _extra: true,
      }))).forEach((g, i) => {
        if (i) card.appendChild(div('grid-divider-h'));
        const t = list.find(x => x.groupId === g.id);
        const cell = div('grid-cell-stack cand-row');
        const head = div('cand-row-head');
        head.appendChild(txt('span', 'cand-row-name', _groupLabel(g)));
        const info = txt('span', 'cand-i', 'i');
        if (g._extra) info.style.display = 'none';
        info.title = `호차구성: ${g.cars_label} (총 ${g.seats}석)\n해당열차: ${g.trains.join(', ')}`;
        info.addEventListener('click', ev => {   // 모바일은 마우스오버가 없다 — 눌러도 뜬다
          ev.stopPropagation();
          _infoSheet(_groupLabel(g), [
            ['호차구성', `${g.cars_label} (총 ${g.seats}석)`],
            ['해당열차', g.trains.join(', ')],
          ]);
        });
        ...
        cell.appendChild(seats.length
          ? txt('div', 'cand-row-seats', seats.map(_seatLabel).join(', '))
          : txt('div', 'cand-row-empty', '선호 좌석을 등록하세요'));
        cell.addEventListener('click', () => _candEditor(
          t || { groupId: g.id, trainType: g.train_type, updown: g.updown, fallback: 'watch', seats: [] },
