...
function _payFields(p) {
  ...
  const pinCard = makeGridCard(); pinCard.appendChild(pinRow); wrap.appendChild(pinCard);
  ...
  function syncMethod() {
    ...
    [pinCard, optLabel, optCard, rcptLabel, receipt.el].forEach(e => e.style.display = cash ? '' : 'none');
...
const _PAY_NOTE_CASH_HTML =
  '계좌 1회 등록으로 현금 결제를 할 수 있는 서비스입니다<br>'
  + '<a href="https://www.korail.com" target="_blank" rel="noopener">코레일 사이트</a>'
  + ' - 마이페이지 - 결제관리 - 간편현금결제 설정에서 등록하세요';
...
function _payMethodNote(getMethod, hasPin) {
  ...
  function paint() {
    const m = getMethod();
    if (m === 'cash') {
      if (hasPin && hasPin()) { el0.style.display = 'none'; return; }
      set('note-line muted', _PAY_NOTE_CASH_HTML); return;
    }
    ...
    _tossKeyState().then(ok => {
      ...
      if (ok === null) { set('note-line muted', '토스페이 등록 여부를 확인하지 못했습니다'); return; }
      set(ok ? 'note-line' : 'warn-line',
          ok ? _PAY_NOTE_TOSS_OK : _PAY_NOTE_TOSS_NONE.replace(/\n/g, '<br>'));
    });
  }
  return { el: el0, paint };
}
...
function _ktxPayModal(onSaved) {   // 결제 정보 모달 — 저장은 계정 레코드의 pay로(lsSetPay)
  ...
  _editModal('결제 정보', (body) => {
    ...
    pinCard = makeGridCard(); pinCard.appendChild(pinRow); body.appendChild(pinCard);
  ...
  _renderUpdateCard = (st) => {
    ...
    if (st.needs_exe && newer) {
      ...
      b.addEventListener('click', async () => {
        ...
        if (r.manual) {
          // 스스로 되살릴 수 없는 환경 — 서버는 그대로 살아 있으니 안내만 한다
          b.textContent = '적용 완료';
          st2.appendChild(txt('div', 'warn-line', '적용했습니다.\n프로그램 창을 닫았다가 다시 실행해 주세요.'));
          notify('업데이트 적용 완료 — 창을 닫았다 다시 열어주세요');
          return;
        }
        b.textContent = '다시 시작하는 중…';
        // 새 프로세스가 뜰 때까지 기다렸다가 살아나면 새로고침
        for (let i = 0; i < 30; i++) {
          await new Promise(res => setTimeout(res, 1000));
          const v = await api('GET', '/version').catch(() => null);
          if (v && v.ok) { location.reload(); return; }
        }
        b.textContent = '창을 닫았다 다시 열어주세요';
      });
      st2.appendChild(b);
    } else {
      ...
      b.addEventListener('click', async () => {
        ...
        // 뒤에서 이미 확인이 돌고 있으면 즉시 checking:true 로 돌아온다.
        // 그대로 그리면 '확인하는 중…'에서 멈춘 것처럼 보이므로, 끝날 때까지 상태를 다시 묻는다.
        ...
        else if (r && r.error) notify(`확인 실패: ${r.error}`);
      });
      st2.appendChild(b);
    }
...
function _boot() {
  ...
  // 부팅 직후 백그라운드 확인이 도는 동안에는 잠깐 자주 본다(설정 화면이 '확인 중'에 갇히지 않게)
  let _updFast = 0;
  const _updTick = setInterval(() => {
    if (++_updFast > 40) return clearInterval(_updTick);
    _pollUpdate();
  }, 3000);
  refreshHeaderPill();
