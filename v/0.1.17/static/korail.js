...
function buildResultsPage() {
  ...
  function fill(c, j) {
    ...
    const state = (j.remote ? 'remote:' : '') + (j.running ? 'run' : (won ? 'won' : 'stop'));
    ...
    if (j.remote) {   // 웹(맥미니)에서 도는 감시 — 이 PC에서는 보기만 한다
      const tag = txt('span', 'job-remote-tag', '웹'); tag.title = '웹에서 실행 중인 감시입니다. 중단·삭제는 웹에서 하세요';
