...
function _buildModePanel(panel, cfg) {
  ...
  // 선호 옵션은 '설정 탭 값'에서 시작한다. 여기서 바꾼 값은 그 화면에서만 쓰이고,
  // 새로고침하면 설정값으로 돌아온다(2026-09-10 지시). 되돌리기는 우측 갱신 아이콘.
  const _pref0 = lsGetSeatPref();
  const seatF = makeSheetPicker(_SEAT_OPTS, _pref0.seatClass, () => save());
  ...
  const waitCb = makeCheckbox(_pref0.allowWaiting); waitCb.addEventListener('change', () => { save(); _syncEtcNotes(); });
  const _standMsg = '좌석이 매진이고 입석만 남은 열차도 자동예매합니다.\n⚠️ 실험적 기능: 좌석이 보장되지 않고 자동결제도 미적용입니다.';
  const standingCb = makeCheckbox(_pref0.allowStanding); standingCb.addEventListener('change', () => { save(); _syncEtcNotes(); });
  ...
  const sideF = makeSheetPicker(_SIDE_OPTS, _pref0.seatSide, () => { renderEtc(); save(); });
  const dirF = makeSheetPicker(_DIR_OPTS, _pref0.seatDir, () => { renderEtc(); save(); });
  ...
  const reset = el('button', 'etc-gear'); reset.type = 'button'; reset.title = '설정한 기본값으로 되돌리기';
  reset.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"'
    + ' stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 3-6.7"/><path d="M3 4v5h5"/></svg>';
  reset.addEventListener('click', ev => {
    ev.stopPropagation();
    const p0 = lsGetSeatPref();
    seatF.setValue(p0.seatClass); sideF.setValue(p0.seatSide); dirF.setValue(p0.seatDir);
    waitCb.checked = p0.allowWaiting; standingCb.checked = p0.allowStanding;
    renderEtc(); _syncEtcNotes(); save();
    notify('설정한 선호 옵션으로 되돌렸습니다.', reset);
  });
  const etcWrap = div('etc-wrap'); etcWrap.append(etcVal, gear, reset);   // 칩 · 수정 · 되돌리기
  const etcCell = addRow(seatCard, 'etc', '선호 옵션', etcWrap);
  // 선호 좌석 모드 전용 — 켜면 실패 시 이 조건으로 예매, 끄면 선호 좌석이 나올 때까지 기다린다
  let candFallback = true;
  const etcToggle = el('button', 'switch on'); etcToggle.type = 'button';
  etcToggle.setAttribute('role', 'switch');
  etcToggle.innerHTML = '<span class="switch-knob"></span>';
  etcToggle.addEventListener('click', ev => { ev.stopPropagation(); candFallback = !candFallback; renderEtc(); save(); });
  const etcWait = txt('span', 'etc-wait', '선호 좌석이 나올 때까지 반복합니다');
  etcWrap.insertBefore(etcWait, etcVal);
  etcCell.querySelector('.grid-cell-label').appendChild(etcToggle);
  etcCell.style.cursor = 'pointer';
  etcCell.addEventListener('click', () => openEtcSheet());
  function renderEtc() {
    const off = (seatMode() === 'cand') && !candFallback;   // 끈 상태: 조건 대신 안내 문구만
    etcToggle.classList.toggle('on', candFallback);
    etcToggle.setAttribute('aria-checked', candFallback ? 'true' : 'false');
    etcToggle.title = candFallback ? '실패 시 이 조건으로 예매' : '선호 좌석이 나올 때까지 반복';
    etcWait.style.display = off ? '' : 'none';
    etcVal.style.display = off ? 'none' : '';
    gear.style.display = off ? 'none' : '';
  ...
  function syncSections() {
    ...
    etcToggle.style.display = (seatMode() === 'cand') ? '' : 'none';   // 토글은 선호 좌석 모드에서만
  ...
  function getStartBody() {
    ...
    if (isManual()) {
      ...
      base.cand_fallback = candFallback ? 'auto' : 'watch';   // 끄면 선호 좌석이 나올 때까지 기다린다
...
function buildSettingsPage() {
  ...
  panel.appendChild(txt('div', 'group-header', '추가 기능'));
  const extCard = makeGridCard();
  const gcCell = div('grid-cell-full acct-row');
  const gcVal = txt('div', 'acct-state', '확인 중…');
  gcCell.append(txt('div', 'grid-cell-label', '구글 캘린더 연동'), gcVal);
  gcCell.addEventListener('click', () => _gcalSheet(paintGcal));
  extCard.append(gcCell, div('grid-divider-h'));
  const mlCell = div('grid-cell-full acct-row');
  const mlVal = txt('div', 'acct-state', '확인 중…');
  mlCell.append(txt('div', 'grid-cell-label', '메일 알림'), mlVal);
  mlCell.addEventListener('click', () => _mailSheet(paintMail));
  extCard.appendChild(mlCell);
  panel.appendChild(extCard);
  let mlOn = false, mlReady = true, mlWhy = '';
  function paintMail() {
    api('GET', '/mail').then(d => {
      mlOn = !!(d && d.enabled); mlReady = !!(d && d.available); mlWhy = (d && d.reason) || '';
      mlVal.textContent = mlOn ? '사용 중' : (mlReady ? '사용 안 함' : '준비 중');
      mlVal.style.color = mlOn ? 'var(--blue)' : 'var(--text-muted)';
    }).catch(() => { mlVal.textContent = '사용 안 함'; });
  }
  paintMail();
  function _mailSheet(onDone) {
    const ov = div('picker-overlay'); const sheet = div('picker-sheet'); sheet.classList.add('u-sheet-pad');
    sheet.appendChild(txt('div', 'wheel-title', '메일 알림'));
    const box = div('grid-cell-stack');
    box.appendChild(txt('div', 'note-line', _byPeriod(
      '예매에 성공하면 로그인한 구글 메일로 알려 드립니다. 열차·좌석·결제기한이 함께 담깁니다.')));
    if (!mlReady) box.appendChild(txt('div', 'note-line muted', mlWhy || '아직 준비 중입니다.'));
    sheet.appendChild(box);
    const bar = div('preset-actions'); bar.style.marginTop = '14px';
    const cancel = el('button', 'btn-primary'); cancel.textContent = '닫기'; cancel.classList.add('btn-ghost');
    cancel.addEventListener('click', () => document.body.removeChild(ov));
    const go = el('button', 'btn-primary');
    go.textContent = mlOn ? '끄기' : '켜기';
    if (mlOn) go.classList.add('btn-ghost', 'btn-danger');
    go.disabled = !mlReady && !mlOn;
    go.addEventListener('click', async () => {
      const r = await api('POST', '/mail', { on: !mlOn }).catch(() => null);
      if (!r || !r.ok) { notify((r && r.error) || '바꾸지 못했습니다.'); return; }
      document.body.removeChild(ov); onDone && onDone();
      notify(r.enabled ? '메일 알림을 켰습니다. 확인 메일을 보냈습니다.' : '메일 알림을 껐습니다.');
    });
    bar.append(cancel, go); sheet.appendChild(bar);
    ov.appendChild(sheet);
    ov.addEventListener('click', e => { if (e.target === ov) document.body.removeChild(ov); });
    document.body.appendChild(ov);
  }
  ...
  function paintGcal() {
    api('GET', '/gcal').then(d => {
      gcOn = !!(d && d.enabled);
      gcVal.textContent = gcOn ? '사용 중' : '사용 안 함';
      gcVal.style.color = gcOn ? 'var(--blue)' : 'var(--text-muted)';
    }).catch(() => { gcVal.textContent = '사용 안 함'; });
  }
  paintGcal();
  function _gcalSheet(onDone) {
    const ov = div('picker-overlay'); const sheet = div('picker-sheet'); sheet.classList.add('u-sheet-pad');
    sheet.appendChild(txt('div', 'wheel-title', '구글 캘린더 연동'));
    const box = div('grid-cell-stack');
    box.appendChild(txt('div', 'note-line', _byPeriod(
      '예매한 승차권이 구글 캘린더에 자동으로 올라갑니다. 취소하면 일정도 함께 지워집니다.')));
    box.appendChild(txt('div', 'note-line muted', '일정 제목은 이렇게 표시됩니다.'));
    box.appendChild(txt('div', 'gcal-sample', '[기차] 용산→광주송정 / 7호차 12A / KTX 405'));
    box.appendChild(txt('div', 'note-line muted', _byPeriod(
      '켤 때 구글에서 캘린더 권한을 한 번 물어봅니다. 일정 만들기 외에는 쓰지 않습니다.')));
    sheet.appendChild(box);
    const bar = div('preset-actions'); bar.style.marginTop = '14px';
    const cancel = el('button', 'btn-primary'); cancel.textContent = '닫기'; cancel.classList.add('btn-ghost');
    cancel.addEventListener('click', () => document.body.removeChild(ov));
    const go = el('button', 'btn-primary');
    go.textContent = gcOn ? '연동 해제' : '연동하기';
    if (gcOn) go.classList.add('btn-ghost', 'btn-danger');
    go.addEventListener('click', async () => {
      if (gcOn) {
        if (!(await askConfirm('구글 캘린더 연동을 끌까요? 이미 올린 일정은 그대로 남습니다.', go))) return;
        await api('POST', '/gcal/off').catch(() => null);
        document.body.removeChild(ov); onDone && onDone();
        notify('구글 캘린더 연동을 껐습니다.');
        return;
      }
      location.href = '/auth/calendar';   // 구글 동의 화면으로
    });
    bar.append(cancel, go); sheet.appendChild(bar);
    ov.appendChild(sheet);
    ov.addEventListener('click', e => { if (e.target === ov) document.body.removeChild(ov); });
...
function _adminSheet() {
  ...
  function mailBox() {   // 주요 업데이트를 알림 켠 사람들에게 보낸다
    const wrap = div('');
    const head = txt('div', 'group-header', '공지 메일');
    wrap.appendChild(head);
    const card = makeGridCard(); const cell = div('grid-cell-stack');
    const info = txt('div', 'note-line muted', '받는 사람 확인 중…');
    const subj = makeTextInput('', 200, '제목');
    const bodyIn = document.createElement('textarea');
    bodyIn.className = 'field-input mail-body'; bodyIn.rows = 5; bodyIn.placeholder = '내용';
    const send = el('button', 'btn-primary'); send.textContent = '보내기'; send.style.marginTop = '10px';
    send.addEventListener('click', async () => {
      if (!subj.value.trim() || !bodyIn.value.trim()) { notify('제목과 내용을 모두 적어 주세요.', send); return; }
      if (!(await askConfirm('알림을 켠 분들에게 메일을 보낼까요?', send))) return;
      send.disabled = true; send.textContent = '보내는 중…';
      const r = await api('POST', '/admin/mail', { subject: subj.value, body: bodyIn.value }).catch(() => null);
      send.disabled = false; send.textContent = '보내기';
      if (!r || !r.ok) { notify((r && r.error) || '보내지 못했습니다.'); return; }
      notify(`${r.sent}/${r.total}명에게 보냈습니다.`);
      subj.value = ''; bodyIn.value = '';
    });
    cell.append(info, subj, bodyIn, send); card.appendChild(cell); wrap.appendChild(card);
    api('GET', '/admin/mail').then(d => {
      if (!d || !d.ok) return;
      info.textContent = d.ready
        ? `메일 알림을 켠 ${d.count}명에게 보냅니다 (발신 ${d.sender})`
        : '보내는 계정이 아직 설정되지 않았습니다';
      send.disabled = !d.ready || !d.count;
    }).catch(() => {});
    return wrap;
  }
  function load() {
