...
_hub_state = {"ok": False, "email": "", "error": "", "at": 0.0}
...
async def _hub_call(method: str, path: str, body=None):
    ...
    try:
        ...
        return None
    if not j.get("ok"):
        _hub_state.update(ok=False, error=str(j.get("error") or "요청 거부"), at=time.time())
        return None if j.get("auth") is False else j
    _hub_state.update(ok=True, error="", at=time.time())
    return j
...
async def api_sync_set(request: Request):
    ...
    if LOCAL_MODE:   # exe — 같은 내용을 중앙에도 올려 웹·다른 PC와 맞춘다(실패해도 로컬은 이미 저장됨)
        await _hub_call("POST", "/api/device/sync", {"data": data, "ts": ts_in})
    if key == _SYNC_KEY:
...
async def api_jobs(request: Request):
    ...
    if LOCAL_MODE:   # exe — 웹(맥미니)에서 돌고 있는 내 감시도 함께 보여 준다(읽기 전용)
        j = await _hub_call("GET", "/api/device/jobs")
        if j and isinstance(j.get("jobs"), list):
            have = {x["job"] for x in out}
            out += [x for x in j["jobs"] if x.get("job") not in have]
    return {"ok": True, "jobs": out}
...
async def api_calllog(request: Request, after: int = 0):
    ...
    return {"ok": True, "calls": get_call_log(after)}
