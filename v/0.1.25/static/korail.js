...
function renderSeatTable(host, seats, opts) {
  ...
  // 열이 나오는 순서(A→D / D→A)는 그 호차가 실제로 놓인 방향이라 그대로 둔다.
  ...
  // 통로는 '몇 번째 열'이 아니라 '묶음이 바뀌는 자리'에 넣는다.
  // 코레일 열 표기는 위치 고정이다 — A·B가 한 묶음, C(·D)가 통로 건너편(2+2 일반실, 2+1 특실 공통).
  // 열 개수로 가운데를 잡으면 방향이 뒤집힌 호차에서 통로가 반대편에 들어가, 특실이 'C B | A'가 아니라
  // 'C B | A' 대신 'CB'를 붙여 그리게 된다(2026-09-10 실측: 캐시 34건 중 16건이 역순 호차).
  const _pair = c => (c === 'A' || c === 'B') ? 1 : 2;
...
function buildSettingsPage() {
  ...
  const outCell = div('grid-cell-full acct-row');
  const outVal = txt('div', 'acct-state', '계정 정보 지우기'); outVal.style.color = 'var(--red)';
  outCell.append(txt('div', 'grid-cell-label', '코레일 로그아웃'), outVal);
  outCell.addEventListener('click', async () => {
    if (!lsGetAccount('ktx').memberNo) { notify('등록된 코레일 계정이 없습니다'); return; }
    if (!(await askConfirm('코레일 계정 정보를 지울까요?\n연결된 기기에서도 다시 로그인해야 합니다.', outCell))) return;
    ktxLogout();
  ...
  panel.appendChild(div('sp18'));
  panel.appendChild(txt('div', 'group-header', '선호 옵션'));
  const prefCard = makeGridCard();
  const prefChips = [];
  const prefRow = (label, opts, key) => {
    if (prefCard.children.length) prefCard.appendChild(div('grid-divider-h'));
    const cell = div('grid-cell-full'); cell.classList.add('chip-row');
    const chip = makeChipGroup(opts, () => lsGetSeatPref()[key], v => lsSetSeatPref({ [key]: v }));
    prefChips.push(chip);
    const act = div('grid-cell-action'); act.appendChild(chip.el);
    cell.append(txt('div', 'grid-cell-label', label), act);
    prefCard.appendChild(cell);
  };
  const prefToggle = (label, key, note) => {
    prefCard.appendChild(div('grid-divider-h'));
    const cell = div('grid-cell-full');
    const cb = makeCheckbox(lsGetSeatPref()[key]);
    cb.addEventListener('change', () => lsSetSeatPref({ [key]: cb.checked }));
    const act = div('grid-cell-action'); act.appendChild(cb);
    cell.append(txt('div', 'grid-cell-label', label), act);
    prefCard.appendChild(cell);
    if (note) prefCard.appendChild(txt('div', 'etc-note', note));
  };
  prefRow('좌석등급', _SEAT_OPTS, 'seatClass');
  prefRow('창측 / 내측', _SIDE_OPTS, 'seatSide');
  prefRow('진행 방향', _DIR_OPTS, 'seatDir');
  prefToggle('예약대기 허용', 'allowWaiting', '좌석이 없으면 예약대기로 걸어 둡니다(자동결제는 안 됩니다).');
  prefToggle('입석 허용', 'allowStanding', '좌석이 없으면 입석으로 잡습니다.');
  panel.appendChild(prefCard);
  panel.appendChild(txt('div', 'note-line muted', '예매 탭의 선호 옵션이 이 값으로 시작합니다. 예매 탭에서 바꾸면 그 감시에만 적용됩니다.'));
  panel.appendChild(div('sp18'));
  ...
  // 부팅 때는 부르지 않는다 — 설정 탭을 열 때 switchTab 이 부른다(코레일 호출 1건·2초 절약)
