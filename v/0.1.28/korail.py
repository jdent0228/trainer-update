...
def _fmt_params(params) -> str:
    ...
    return " ".join(parts)
# 어느 감시 작업이 낸 호출인지(빈 값 = 화면 조회). runner_loop이 작업 시작 때 설정한다(QA 호출 귀속용, 2026-09-10)
CUR_JOB: "_contextvars.ContextVar[str]" = _contextvars.ContextVar("ktx_job", default="")

def _log_call(method: str, url: str, params, acct: str = "") -> None:
    try:
        _CALL_SEQ[0] += 1
        ...
        item = {"n": _CALL_SEQ[0], "t": datetime.now().strftime("%H:%M:%S"),
                ...
                "ep": short, "url": url, "params": _fmt_params(params), "job": CUR_JOB.get(""),
                "acct": acct or "", "cached": False}
        with _CALL_LOCK:
            _CALL_LOG.append(item)
    except Exception:
        pass
def log_cache_hit(purpose: str, detail: str = "", acct: str = "", job: str = "") -> None:
    """코레일에 나가지 않고 캐시로 답한 조회도 같은 줄에 남긴다 — 실제 호출과 구분해서 표시한다."""
    try:
        _CALL_SEQ[0] += 1
        item = {"n": _CALL_SEQ[0], "t": datetime.now().strftime("%H:%M:%S"),
                "purpose": purpose, "method": "CACHE", "host": "cache", "ep": "(캐시)",
                "url": "", "params": detail or "", "job": job or CUR_JOB.get(""),
                "acct": acct or "", "cached": True}
        with _CALL_LOCK:
            _CALL_LOG.append(item)
    except Exception:
        pass
...
def _guard_session(sess, acct: str = ""):
    """requests.Session의 모든 요청을 관문에 통과시킨다(get/post 모두 request()를 거친다).
    acct = 이 세션이 쓰는 코레일 계정 — 호출 로그에 '누가 보냈는지' 남기려고 함께 붙인다(2026-09-10)."""
    if sess is None:
        return sess
    if getattr(sess, "_gated", False):
        if acct:
            sess._acct = acct
        return sess
    sess._acct = acct
...
_LOGIN_BLOCK_AFTER = 3   # 코레일은 5회 연속 오류에서 계정을 잠근다 — 그 앞에서 멈춘다
...
def login_id_kind(v: str) -> str:
    """로그인 ID의 종류: 'email' | 'phone' | 'member' | '' (형식 아님).
    형식이 아니면 코레일에 물어볼 필요가 없다 — 헛시도는 계정 잠금 위험만 키운다(2026-09-10)."""
    v = (v or "").strip()
    if not v:
        return ""
    if "@" in v:
        return "email" if re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", v) else ""
    d = re.sub(r"\D", "", v)
    if len(d) != len(re.sub(r"[\s-]", "", v)):   # 숫자·하이픈·공백 외 문자가 섞였다
        return ""
    if d.startswith("01") and len(d) in (10, 11):
        return "phone"
    if 8 <= len(d) <= 11:
        return "member"
    return ""
...
def norm_login_id(v: str) -> str:
    """코레일 로그인 ID 정규화. korail2 는 정규식으로 종류를 가른다:
    이메일(@ 포함) → txtInputFlg 5, `010-1234-5678` 꼴 → 4, 그 외 → 회원번호(2).
    사용자는 보통 하이픈 없이 01012345678 로 적는데 그러면 회원번호로 보내져
    'WRT200320 잘못 입력하셨습니다(회원번호)' 로 실패한다(2026-09-10 실측). 여기서 맞춰 준다."""
    v = (v or "").strip()
    if not v or "@" in v:
        return v
    d = re.sub(r"\D", "", v)
...
def _make_korail(member_no=None, password=None) -> Korail:
    ...
    if acct and not login_id_kind(acct):
        raise KorailError("회원번호 또는 휴대폰번호 형식이 아닙니다. 다시 확인해 주세요", "LOGIN_ID_FORMAT")
    left = login_block_left(acct)
    if left:
        raise KorailError(
            f"로그인 실패가 반복돼 {left}초간 시도를 멈췄습니다(계정 잠금 방지). "
            "설정에서 회원번호·비밀번호를 확인해 주세요.", "LOGIN_COOLDOWN")
    korail = Korail(norm_login_id(member_no or KORAIL_ID), password or KORAIL_PW, auto_login=False)
    ...
    _guard_session(korail._session, acct)
    last, bad_cred = "", False
    for attempt in range(1, 4):
        ...
        # S034=비밀번호 오류, WRT200320=회원번호(ID) 오류. 둘 다 재시도가 무의미하다.
        bad_cred = "S034" in last or "WRT200320" in last
    ...
    if "WRT200320" in str(last):
        raise KorailError("회원번호(또는 휴대폰번호)가 코레일에 없습니다. 다시 확인해 주세요", "LOGIN_ID")
    if "S034" in str(last):
        raise KorailError("비밀번호가 틀렸습니다. 다시 확인해 주세요", "LOGIN_PW")
...
def is_not_open_yet(e) -> bool:
    ...
    # WRD000058 '승차권 예약발매 준비 중'(1개월 밖) / WRD000052 '발매 개시 준비 중'(자정 넘어 1개월 안에 들어왔지만
    # 개시 시각 전). 후자를 못 알아봐 개시일 새벽 0시부터 10초마다 헛조회했다(2026-09-10 00:52 실측, QA 발견).
    code = str(getattr(e, "code", "") or "")
    msg = str(getattr(e, "msg", "") or getattr(e, "args", [""])[0] or "")
    return code in ("WRD000058", "WRD000052") or "발매 준비 중" in msg or "개시 준비 중" in msg
...
def _build_candidates(korail: Korail, p: Dict[str, Any]) -> List[Tuple]:
    ...
    for train in trains:
        ...
        # 사용자가 열차를 직접 고른 경우엔 시간대 필터를 적용하지 않는다 — 고른 것이 곧 대상이다.
        # (2026-09-10 사고: 감시 범위 13:00~14:00에 14:09 출발 422편을 고르자 매 사이클 탈락해
        #  10시 예매 개시 후에도 "후보 없음"만 반복하고 예약을 못 했다. 목록은 시작시각부터 10편을
        #  보여주므로 범위 밖 열차를 고르는 것이 정상 동작이다.)
        if not only_nos and not (start_min <= dep_min <= end_min):
            continue
        if len(watching) < 20:
            watching.append({"no": train.train_no, "type": train.train_type_name,
                             "dep_time": train.dep_time, "arr_time": train.arr_time})
        if max_dur > 0 and not only_nos:   # 직접 고른 열차는 소요시간 조건보다 우선한다
        ...
        # 예약대기는 사용자가 허용한 경우에만 후보로 삼는다(이전 버그: allow_waiting을 읽기만 하고 안 써서
        # 미허용인데도 매진 열차에 예약대기를 등록했다 — QA 하네스 2026-09-10 발견)
        is_waiting = allow_waiting and (not has_seat) and train.has_waiting_list()
...
def align_cand_cars(korail: Korail, train, seat: str, want: List[dict], p: dict) -> List[dict]:
    ...
    if key not in cache:
        ...
        cache[key] = nos or None
    nos = cache[key]
    if not nos:
        return want
    # 산천 8량 편성만 번호대(1~8 ↔ 11~18)가 바뀐다. KTX 18량(1~18)은 12호차가 실제로 있으므로 옮기면 안 된다
    # (이전 버그: min(호차)>=11 만 보고 18량 KTX의 12~18호차 후보를 2~8호차로 옮겨 1순위를 놓쳤다 — QA 2026-09-10).
    # 규칙: 원하는 호차가 실제 목록에 있으면 그대로, 없는데 ±10 한 호차가 있으면 그쪽으로.
    out, moved = [], 0
    for x in want:
        ...
        n = int(raw)
        if n not in nos:
            alt = n + 10 if n <= 8 else (n - 10 if n >= 11 else n)
            if alt in nos:
                n = alt; moved += 1
        out.append({**x, "car_no": str(n).zfill(len(raw)) if len(raw) >= 4 else str(n)})
    if moved:
        logger.info(f"{train.train_no}편 편성({min(nos)}~{max(nos)}호차)에 맞춰 후보 좌석 호차 {moved}건 옮김")
    return out
...
def _web_post(path: str, data: dict, acct: str = "") -> dict:
    _log_call("POST", WEB_BASE + path, data, acct=acct)
    with GATE:
...
def list_cars_web(korail: Korail, train, seat_class: str = "1", count: int = 1) -> List[dict]:
    ...
    j = _web_post(".research.TrainResearch", d, acct=getattr(korail._session, "_acct", ""))
...
def seat_map_web(korail: Korail, train, car_no: str, seat_class: str = "1", count: int = 1) -> dict:
    ...
    j = _web_post(".research.TResidualSeatsResearch.do", dict(_web_common(korail, train, seat_class, count), srcarNo=car_no),
                  acct=getattr(korail._session, "_acct", ""))
    ...
    return {"car_no": str(j.get("scar_no") or car_no),
            ...
            # seat_ary_cd 는 코레일이 좌석 배치를 알려주는 공식 코드다('3'=2+1 특실, '4'=2+2 일반실).
            # 화면이 통로 위치를 열 개수로 짐작하지 않도록 그대로 넘긴다(2026-09-10).
            "layout": "2+1" if ary == "3" else "2+2", "seat_ary_cd": ary,
...
def pick_seats(korail: Korail, train, seat_class: str, count: int, prefs: dict, stats: Optional[dict] = None) -> List[dict]:
    ...
    def score(x):
        ...
        return sc
    # 달성 가능한 최고점: 옆자리 묶음은 창측이 한 자리뿐이므로 창측 4점은 1회만(2명 이상). 방향 점수는 인원수만큼.
    # (이전: 4*count 로 잡아 2명일 때 만점이 불가능 → 조기 종료 없이 최대 6호차 좌석맵을 다 훑었다. QA 2026-09-10)
    max_sc = (4 if side in ('window', 'aisle') else 0) * (count if side == 'aisle' else 1) + (2 if want_dir else 0) * count
...
def list_ncards(korail: Korail, raw: Optional[dict] = None) -> List[dict]:
...
def _reservation_list_raw(korail: Korail) -> dict:
    """예약 목록(ReservationView) 원본 응답 1회. my_tickets가 결제기간·예약 객체 양쪽에 재사용한다(중복 1콜 제거, QA 2026-09-10)."""
    url = _k2.KORAIL_MYRESERVATIONLIST
    data = {"Device": korail._device, "Version": korail._version, "Key": korail._key}
    headers, sid = korail._get_auth_headers_and_sid(url)
    if sid:
        data["Sid"] = sid
    return json.loads(korail._session.post(url, data=data, headers=headers).text)


def _reservation_pay_window(korail: Korail, raw: Optional[dict] = None) -> Dict[str, dict]:
    ...
    out: Dict[str, dict] = {}
    try:
        j = raw if raw is not None else _reservation_list_raw(korail)
...
def my_tickets(korail: Korail) -> dict:
    ...
    raw_rsv = None; raw_tk = None
    try:
        raw_rsv = _reservation_list_raw(korail)
    except Exception as e:
        logger.debug(f"[예약] 목록 조회 실패: {e}")
    pay_win = _reservation_pay_window(korail, raw=raw_rsv) if raw_rsv is not None else {}
    def _rsvs():
        # korail2.reservations()와 같은 파싱을 이미 받은 응답에 적용(같은 엔드포인트를 두 번 부르지 않는다)
        if raw_rsv is None:
            return korail.reservations()
        try:
            if not korail._result_check(raw_rsv):
                return []
        except _k2.NoResultsError:
            return []
        return [_k2.Reservation(ti) for jr in ((raw_rsv.get("jrny_infos") or {}).get("jrny_info") or [])
                for ti in ((jr.get("train_infos") or {}).get("train_info") or [])]
    try:
        for r in _rsvs():
    ...
    try:
        ...
        if sid:
            data["Sid"] = sid
        j = json.loads(korail._session.post(url, data=data, headers=headers).text)
        raw_tk = j   # N카드 목록도 같은 응답에서 뽑는다(list_ncards 재조회 제거)
        def _n(v):
            try: return int(str(v or "0").lstrip("0") or "0")
            except Exception: return 0
    ...
    try:
        out["ncards"] = list_ncards(korail, raw=raw_tk)
    except Exception as e:
...
def history_clusters(rides: List[dict], gap_min: int = HIST_CLUSTER_GAP_MIN,
    ...
    for (dep, arr, wd), items in groups.items():
        ...
        for r in items + [None]:
            ...
            if r is not None:
                run.append(r)
    # 3회 이상 묶음이 하나도 없으면(요일이 흩어졌거나 아직 기록이 적을 때) 그냥 비워 두지 않고
    # 많이 탄 구간 상위 2개를 넣어 준다 — 요일은 전부 같을 때만 붙인다(2026-09-10 지시).
    if not out:
        by_route: Dict[Tuple[str, str], List[dict]] = {}
        for r in rides:
            by_route.setdefault((r["dep"], r["arr"]), []).append(r)
        ranked = sorted(by_route.items(), key=lambda kv: (-len(kv[1]), kv[0]))
        for (dep, arr), items in ranked[:2]:
            wds = {r["weekday"] for r in items}
            out.append(_cluster_to_preset(dep, arr, wds.pop() if len(wds) == 1 else None,
                                          sorted(items, key=lambda r: r["minutes"])))

    # 많이 탄 순 → 요일 → 시각
    out.sort(key=lambda c: (-c["rides"], 9 if c["weekday"] is None else c["weekday"], c["start_min"]))
    return out


def _cluster_to_preset(dep: str, arr: str, wd: Optional[int], run: List[dict]) -> dict:
    ...
    return {
        "dep": dep, "arr": arr,
        "weekday": None if wd is None else (wd + 1) % 7,   # 0=월(파이썬) → 0=일(JS). None=요일 무관
        "rides": len(run),
